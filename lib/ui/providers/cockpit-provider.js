...
const backupState = {
  ...
  lastFailure: null,
  recentEvents: [],
  status: 'unknown', // 'protected' | 'stale' | 'unprotected' | 'unknown'
};
...
function updateBackupState(event) {
  if (event.type === 'backup') {
    ...
    const hasBackupErrors = false; // TODO: Check error registry for backup errors

    if (event.success && hasRecentSuccess && !hasBackupErrors) {
      backupState.status = 'protected';
    } else if (backupState.lastSuccess && !hasRecentSuccess) {
      backupState.status = 'stale';
    } else if (!event.success && !backupState.lastSuccess) {
      backupState.status = 'unprotected';
    } else if (!event.success && backupState.lastSuccess) {
      backupState.status = 'stale';
    } else {
