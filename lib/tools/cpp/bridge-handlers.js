 ...
 *
 * Dependencies are "soft" - features gracefully degrade if clangd/clang-tidy/cppcheck not found.
 *
...
// Dependency health state (updated by dependency checker)
const dependencyHealth = {
  clangd: { available: false, executable: null },
  'clang-tidy': { available: false, executable: null },
  cppcheck: { available: false, executable: null },
};

// Listen for dependency health updates
if (process.send) {
  process.on('message', (msg) => {
    if (msg?.type === 'dependency-health') {
      dependencyHealth.clangd.available = msg.results.clangd?.ok || false;
      dependencyHealth.clangd.executable = msg.results.clangd?.executable || null;
      dependencyHealth['clang-tidy'].available = msg.results['clang-tidy']?.ok || false;
      dependencyHealth['clang-tidy'].executable = msg.results['clang-tidy']?.executable || null;
      dependencyHealth.cppcheck.available = msg.results.cppcheck?.ok || false;
      dependencyHealth.cppcheck.executable = msg.results.cppcheck?.executable || null;
    }
  });
}
...
export const cppBridgeHandlers = {
  get_cpp_errors: async (args) => {
    try {
      ...
      // Check if any C++ tools are available
      const toolsAvailable =
        dependencyHealth.clangd.available ||
        dependencyHealth['clang-tidy'].available ||
        dependencyHealth.cppcheck.available;
          ...
          content: [
            {
              type: 'text',
              text:
                'C++ analysis tools not found (clangd, clang-tidy, cppcheck).\n\n' +
                'To enable C++ analysis:\n' +
                '1. Install tools: https://clangd.llvm.org/installation, https://cppcheck.sourceforge.io/\n' +
                '2. Add to PATH, or set env vars: CLANGD_PATH, CLANG_TIDY_PATH, CPPCHECK_PATH\n' +
      ...
      for (const filePath of cppFiles.slice(0, 50)) {
        ...
        const result = await analyzeCppFile(filePath, {
          useClangTidy: dependencyHealth['clang-tidy'].available,
          useCppcheck: dependencyHealth.cppcheck.available,
        });
        if (result.errors) {
      ...
      const disabledTools = [];
      if (!dependencyHealth['clang-tidy'].available) disabledTools.push('clang-tidy');
      if (!dependencyHealth.cppcheck.available) disabledTools.push('cppcheck');
        ...
        content: [
          {
            type: 'text',
            text:
              `Found ${allErrors.length} C++ errors in workspace.\n\n` +
              (disabledTools.length > 0
                ? `Note: ${disabledTools.join(', ')} not available - analysis limited to pattern-based detection.\n\n`
                : '') +
              allErrors
                .map(
                  (e) =>
