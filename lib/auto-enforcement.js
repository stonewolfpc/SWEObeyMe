...
export class AutoEnforcementEngine {
  constructor(options = {}) {
    ...
    // Resolve maxFileSize: explicit option > per-project config > default 700
    // Use 700 (not 500) as the default to match obey_surgical_plan's global default
    const projectMax = options.projectDir ? loadProjectMaxLines(options.projectDir) : null;
    const defaultMax = projectMax || options.maxFileSize || 700;
