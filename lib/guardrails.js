 ...
 */
export function enforceStrictMode(content, filePath) {
  ...
  // Check all forbidden patterns (JS-centric patterns only apply to JS/TS)
  ...
  const antiPatterns = checkAntiPatterns(content, lang);
  ...
  const naming = validateNamingConventions(content, lang);
  if (!naming.valid) {
 ...
 * Check for anti-patterns
 */
export function checkForAntiPatterns(content, language) {
  return checkAntiPatterns(content, language);
}
