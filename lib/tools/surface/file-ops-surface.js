...
export async function fileOpsHandler(params) {
  ...
  if (!operation) {
    return {
      ...
      content: [
        {
          type: 'text',
          text: 'operation parameter is required (read, write, info, analyze, extract)',
        },
      ],
    };
  }
    ...
    default:
      return {
        ...
        content: [
          {
            type: 'text',
            text: `Unknown operation: ${operation}. Valid: read, write, info, analyze, extract`,
          },
        ],
      };
  }
  ...
  // Use 10s for read/info operations, 30s for write/extract which may need backups
  const timeoutMs =
    operation === 'read' || operation === 'info' || operation === 'analyze' ? 10000 : 30000;
  const timeoutPromise = new Promise((_, reject) =>
    setTimeout(() => reject(new Error(`file_ops ${operation} timeout (${timeoutMs}ms)`)), timeoutMs)
  );
