...
async function buildPackages() {
  ...
  if (process.platform === 'win32') {
    await runCommand(`powershell -Command "Compress-Archive -Path '${releaseDir}' -DestinationPath '${zipPath}' -Force"`);
  } else {
    await runCommand(`cd ${path.dirname(releaseDir)} && zip -r SWEObeyMe-${version}.zip SWEObeyMe-${version}/`);
  }
