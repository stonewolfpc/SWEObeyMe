...
export function enforceStrictMode(content) {
  ...
  if (commentRatio < minRatio) {
    errors.push(`Strict mode: Documentation ratio ${(commentRatio * 100).toFixed(1)}% below strict minimum of ${(minRatio * 100).toFixed(1)}%`);
  }
...
export function checkForSecurityIssues(content) {
  ...
  if (content.includes('SELECT') && content.includes('WHERE') && !content.includes('prepared') && !content.includes('parameterized')) {
    securityIssues.push({
      type: 'sql_injection',
      severity: 'high',
