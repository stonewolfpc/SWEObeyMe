...
export function getProjectAwarenessToolDefinitions() {
  return [
    ...
    },
    {
      name: 'project_rules',
      priority: 90,
      description: 'Swiss-army-knife for project-specific constraint management. Validate actions or get project rules. Use this when: validating file modifications, understanding project constraints, or checking SoC boundaries. Helps prevent violations of project-specific constraints.',
      inputSchema: {
        type: 'object',
        properties: {
          operation: {
    ...
    },
    {
      name: 'project_track',
      priority: 85,
      description: 'Swiss-army-knife for project state tracking. Add pending tasks, warnings, errors, set current task, advance tasks, audit, or archive. Use this when: tracking incomplete work, logging issues, managing task lifecycle, or clearing completed tasks. Helps persist context across sessions.',
      inputSchema: {
        type: 'object',
        properties: {
          operation: {
            type: 'string',
            enum: ['add_task', 'add_warning', 'add_error', 'clear_tasks', 'set_current', 'advance', 'audit', 'archive'],
            description: 'Operation to perform',
          },
          task: { type: 'string', description: 'Task description (for "add_task" operation)' },
          taskId: { type: 'string', description: 'Task ID (auto-generated if not provided, for "add_task" or "set_current" operations)' },
          taskStatus: { type: 'string', enum: ['pending', 'in_progress', 'completed', 'blocked'], description: 'Task status (default: pending, for "add_task" operation)' },
