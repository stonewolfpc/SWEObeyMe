...
const PLATFORM_INFO = {
  ...
  xdgDataHome: process.env.XDG_DATA_HOME || 'not set',
};

// Log platform info on module load for debugging
console.log('[Backup System] Platform Info:', JSON.stringify(PLATFORM_INFO, null, 2));
...
async function ensureBackupDir() {
  ...
  try {
    ...
    console.error(`[Backup] Primary backup directory failed: ${BACKUP_DIR}`, error.message);
  }
  ...
  for (const fallbackPath of fallbackPaths) {
    try {
      ...
      console.log(`[Backup] Using fallback backup directory: ${fallbackPath}`);
      // Update BACKUP_DIR to the successful fallback
      // Note: This is a simple approach - in production you might want to update a global state
      return;
    } catch (fallbackError) {
      console.error(`[Backup] Fallback directory failed: ${fallbackPath}`, fallbackError.message);
    }
  }

  // All paths failed - report error
  const err = {
  ...
  console.error('[Backup] All backup directory paths failed');
}
...
async function ensureSnapshotDir() {
  ...
  try {
    ...
    console.error(`[Snapshot] Primary snapshot directory failed: ${SNAPSHOT_DIR}`, error.message);
  }
  ...
  for (const fallbackPath of fallbackPaths) {
    try {
      await fs.mkdir(fallbackPath, { recursive: true });
      console.log(`[Snapshot] Using fallback snapshot directory: ${fallbackPath}`);
      return;
    } catch (fallbackError) {
      console.error(`[Snapshot] Fallback directory failed: ${fallbackPath}`, fallbackError.message);
    }
  }
  ...
  registerError(err);
  reportErrorToGitHub(err).catch(() => {});
  console.error('[Snapshot] All snapshot directory paths failed');
}
  ...
  try {
    ...
    if (backupChecksum !== checksum) {
      ...
      pushRouterEvent({
        ...
        timestamp: Date.now(),
        success: false,
        detail: 'Backup checksum mismatch',
      });
      return null;
    }
