...
export async function writeFileHandler(args) {
  ...
  try {
    ...
    // Create backup before edit if file exists (ensures backup even when governance router is bypassed)
    if (fileExists) {
      const backupPath = await autoBackupBeforeEdit(args.path);
      if (backupPath) {
