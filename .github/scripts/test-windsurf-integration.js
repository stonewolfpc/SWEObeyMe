...
async function testMCPProtocol() {
  ...
  return new Promise((resolve) => {
    ...
    server.stdout.on('data', (data) => {
      ...
      if (serverOutput.includes('SWEObeyMe') || serverOutput.includes('listening') || serverOutput.includes('ready')) {
        serverReady = true;
      ...
      if (extensionJs.includes(generator)) {
        ...
        if (startIndex !== -1) {
          ...
          if (endIndex !== -1) {
            ...
            if (functionBody.includes('<html') || functionBody.includes('<body') || functionBody.includes('<div')) {
