...
import { getFileRegistry, searchFiles } from "./file-registry.js";
...
class ReferenceValidator {
    ...
    if (language === 'javascript' || language === 'typescript') {
      ...
      while ((match = importRegex.exec(code)) !== null) {
        ...
        if (importPath.startsWith('.')) {
          ...
          result.addReference(reference, exists);
          if (!exists) {
            result.addWarning(`Node module not found: ${importPath} (may need to be installed)`, reference);
          }
        }
      }
      ...
      while ((match = requireRegex.exec(code)) !== null) {
        ...
        if (requirePath.startsWith('.')) {
          ...
          result.addReference(reference, exists);
          if (!exists) {
            result.addWarning(`Node module not found: ${requirePath} (may need to be installed)`, reference);
          }
        }
      }
  ...
  async validateFileReferences(code, filePath) {
    ...
    while ((match = filePattern.exec(code)) !== null) {
      ...
      if (filePathRef.startsWith('http://') || filePathRef.startsWith('https://') || filePathRef.startsWith('data:')) {
        continue;
      }
  ...
  extractDefinedSymbols(code, language) {
    ...
    if (language === 'javascript' || language === 'typescript') {
      ...
      const funcRegex = /(?:function\s+(\w+)|(?:const|let|var)\s+(\w+)\s*=\s*(?:async\s+)?function|(?:const|let|var)\s+(\w+)\s*=\s*\([^)]*\)\s*=>|class\s+(\w+))/g;
      let match;
      ...
      const methodRegex = /(?:public|private|protected|internal)\s+(?:static\s+)?(?:async\s+)?(?:void|\w+)\s+(\w+)/g;
  ...
  isBuiltInSymbol(symbol, language) {
    const builtIns = {
      javascript: ['console', 'document', 'window', 'Math', 'Array', 'Object', 'String', 'Number', 'Boolean', 'Date', 'Promise', 'setTimeout', 'setInterval', 'fetch', 'JSON', 'parseInt', 'parseFloat', 'isNaN', 'isFinite'],
      typescript: ['console', 'document', 'window', 'Math', 'Array', 'Object', 'String', 'Number', 'Boolean', 'Date', 'Promise', 'setTimeout', 'setInterval', 'fetch', 'JSON', 'parseInt', 'parseFloat', 'isNaN', 'isFinite', 'interface', 'type', 'enum'],
      csharp: ['Console', 'String', 'int', 'double', 'float', 'bool', 'void', 'Task', 'List', 'Dictionary', 'Array', 'IEnumerable', 'string', 'var', 'async', 'await', 'return', 'if', 'else', 'for', 'foreach', 'while', 'try', 'catch', 'finally', 'throw', 'new', 'this', 'base', 'typeof', 'as', 'is']
    };
   ...
   */
  async checkCSharpReference(usingStatement, dir) {
  ...
  async validateAllReferences(code, filePath, language) {
    ...
    combined.warnings = [...results.imports.warnings, ...results.references.warnings, ...results.fileReferences.warnings];
    combined.references = [...results.imports.references, ...results.references.references, ...results.fileReferences.references];
    combined.missingReferences = [...results.imports.missingReferences, ...results.references.missingReferences];
    combined.foundReferences = [...results.imports.foundReferences, ...results.references.foundReferences];
