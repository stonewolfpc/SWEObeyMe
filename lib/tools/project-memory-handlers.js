  ...
  try {
    ...
    if (queryType === 'impacts' || queryType === 'all') {
      const filteredImpacts = manager.dependencyImpacts.filter(i => {
        if (relatedTo && !i.sourceFile?.includes(relatedTo) && !i.affectedFiles?.some(f => f.includes(relatedTo))) return false;
        return true;
      });
    ...
    return {
      isError: true,
      content: [{ type: 'text', text: `Failed to query implementation knowledge: ${error.message}` }],
    };
  }
}
