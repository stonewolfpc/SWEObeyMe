...
export const corpusManagementHandlers = {
    ...
    if (!args.type) {
      return {
        ...
        content: [
          {
            type: 'text',
            text: 'ERROR: "type" parameter is REQUIRED. Provide the project type (godot, csharp, cpp, python, node, default).',
          },
        ],
      };
    }
    ...
    if (!validTypes.includes(args.type)) {
      return {
        ...
        content: [
          {
            type: 'text',
            text: `ERROR: Invalid project type "${args.type}". Must be one of: ${validTypes.join(', ')}.`,
          },
        ],
      };
    }
    ...
    if (!args.corpus) {
      return {
        ...
        content: [
          {
            type: 'text',
            text: 'ERROR: "corpus" parameter is REQUIRED. Provide the corpus name (docs, patreon, math, fdq, training, unified, godot, csharp).',
          },
        ],
      };
    }
    ...
    if (!validCorpora.includes(args.corpus)) {
      return {
        ...
        content: [
          {
            type: 'text',
            text: `ERROR: Invalid corpus "${args.corpus}". Must be one of: ${validCorpora.join(', ')}.`,
          },
        ],
      };
    }
  ...
  /**
   * Dispatcher: corpus_manage swiss-army-knife handler
   * Routes to appropriate handler based on operation parameter
   */
  corpus_manage: async (params) => {
    const { operation, type, corpus } = params;
    ...
    switch (operation) {
      case 'set_type':
        return await corpusManagementHandlers.set_project_type({ type });
      case 'enable':
        return await corpusManagementHandlers.enable_corpus({ corpus });
      case 'disable':
        return await corpusManagementHandlers.disable_corpus({ corpus });
      case 'stats':
        return await corpusManagementHandlers.get_tool_registry_stats(params);
      default:
        return {
          isError: true,
