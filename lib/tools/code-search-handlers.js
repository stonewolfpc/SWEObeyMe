...
export async function search_llama_docs(args) {
  ...
  try {
    // Use ide_mcp_corpus directory instead of process.cwd() which resolves to Windsurf installation
    const docsDir = path.join(process.cwd(), 'ide_mcp_corpus');
...
export async function search_math_docs(args) {
  ...
  try {
    // Use ide_mcp_corpus directory instead of process.cwd() which resolves to Windsurf installation
    const mathDir = path.join(process.cwd(), 'ide_mcp_corpus', 'math');
