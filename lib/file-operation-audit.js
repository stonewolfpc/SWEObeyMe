...
class FileOperationAudit {
  ...
  wasRecentlyWritten(filePath, timeWindow = 30000) {
    ...
    const recentWrites = history.filter(op =>
  ...
  getBlockedOperations(limit = 50) {
    return this.operations
      .filter(op => op.status === OperationStatus.BLOCKED || op.status === OperationStatus.DUPLICATE)
      .slice(-limit);
  }
  ...
  getDuplicateOperations(limit = 50) {
    return this.operations
      .filter(op => op.status === OperationStatus.DUPLICATE)
      .slice(-limit);
  }
  ...
  detectIssues() {
    ...
    const recentCreates = this.getOperationsByType(OperationType.CREATE)
      .filter(op => (Date.now() - op.timestamp) < 10000);
