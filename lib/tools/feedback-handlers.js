...
export async function guidance_handler(params) {
  ...
  switch (operation) {
    case 'explain':
      return await feedbackHandlers.explain_rejection({ reason, context });
    case 'alternatives':
      return await feedbackHandlers.suggest_alternatives({ failed_operation, context });
    case 'operation_guide':
      return await feedbackHandlers.get_operation_guidance({ operation: operation_name, context });
    case 'summary':
