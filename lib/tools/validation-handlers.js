    ...
    try {
      ...
      if (operation) {
        switch (operation) {
          ...
          case 'verify_syntax':
            return await validationHandlers.verify_syntax({ code: content || (path ? await fs.readFile(path, 'utf-8') : null), language });
          case 'verify_imports':
            return await validationHandlers.verify_imports({ content: content || (path ? await fs.readFile(path, 'utf-8') : null), path });
          ...
          default:
            return {
              isError: true,
              content: [{ type: 'text', text: `Unknown operation: ${operation}. Valid: dry_run, verify_syntax, verify_imports, check_anti_patterns, validate_naming, comprehensive` }],
            };
        }
      }
