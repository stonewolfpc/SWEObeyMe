    ...
    check: (content, matches) => {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        for (const match of lineMatches) {
          ...
          if (varName === 'this' || varName === 'base' || varName === 'Console' || varName === 'Debug') continue;
          
          // Check if there's a null check in the previous 5 lines
          const prevLines = lines.slice(Math.max(0, i - 5), i);
          const hasNullCheck = prevLines.some(l => 
    ...
    check: (content, matches) => {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (match) {
          ...
          const hasUnsubscribe = classContent.includes(`-=${handlerName}`) || 
    ...
    check: (content, matches) => {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (match) {
          ...
          const inConstructor = prevLines.some(l => l.includes('constructor') || l.includes('static'));
          
          // Check if it's a readonly field
          const fieldDeclaration = lines.slice(Math.max(0, i - 5), i).find(l => l.includes(fieldName));
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (divMatch) {
          ...
          if (!/^\d+$/.test(divisor) && divisor !== '0') {
            ...
            const hasGuard = prevLines.some(l => 
