...
import {
  writeFileHandler,
  extractToNewFileHandler,
  refactorManageHandler,
} from './handlers-file-write.js';
...
export const fileOperationHandlers = {
  ...
  swe_write_file: writeFileHandler,
  ...
  swe_list_directory: async (args) => {
    if (!args.path) {
      return createErrorResponse('swe_list_directory', new Error('"path" parameter is REQUIRED'));
    }
    try {
      const files = await readdirSafe(args.path, {}, 10000, 'swe_list_directory');
      return createSuccessResponse(files.join('\n'));
    } catch (error) {
      return createErrorResponse('swe_list_directory', error, `Listing directory: ${args.path}`);
    }
  },
