...
/** Extract Python/Ruby functions (indentation-based), with class and async awareness */
export function extractPythonFunctions(content) {
  ...
  // Build a map of line -> class name for class body detection
  const classAtLine = {};
  for (let i = 0; i < lines.length; i++) {
    const cm = lines[i].match(classRegex);
    if (cm) classAtLine[i] = { name: cm[2], indent: cm[1].length };
  }

  // For each def, determine if it's inside a class
  function getClassContext(lineIdx, defIndent) {
    let best = null;
    for (const [li, cls] of Object.entries(classAtLine)) {
      const classLine = Number(li);
      if (classLine < lineIdx && cls.indent < defIndent) {
        if (!best || cls.indent > best.indent) best = cls;
      }
    }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const className = getClassContext(i, baseIndent);
    const rawContent = lines.slice(i, endLine + 1).join('\n');
