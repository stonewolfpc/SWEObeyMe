...
function searchCorpus(corpus, query, category, tags) {
  const index = getIndex(corpus.path);
  if (!index) return [];
  ...
  for (const cat of index.categories) {
    ...
    for (const doc of cat.documents) {
      const titleMatch = !lowerQuery || doc.title.toLowerCase().includes(lowerQuery);
...
export const unifiedHandlers = {
  unified_lookup: async (args) => {
    ...
    if (corpus) {
      ...
      const results = searchCorpus(targetCorpus, query, category, tags);
      return {
        success: true,
    ...
    for (const corp of corpora) {
      const results = searchCorpus(corp, query, category, tags);
