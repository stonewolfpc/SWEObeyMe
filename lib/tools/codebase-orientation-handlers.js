...
import { withTimeout, recordModuleAnnotations, recordDependencyImpact } from './codebase-orientation-utils.js';
import { detectProjectType, identifyEntryPoints, inferModuleStructure } from './codebase-orientation-analysis.js';
...
export async function codebase_orientation_handler(args) {
  ...
  try {
    ...
    async function buildTree(dir, prefix = '', depth = 0) {
      ...
      try {
        const items = await withTimeout(fs.readdir(dir, { withFileTypes: true }), 5000, `buildTree readdir ${dir}`);
...
export async function dependency_analysis_handler(args) {
  ...
  try {
    ...
    async function findCodeFiles(dir, depth = 0) {
      ...
      try {
        const items = await withTimeout(fs.readdir(dir, { withFileTypes: true }), 5000, `findCodeFiles readdir ${dir}`);
    ...
    for (const file of codeFiles) {
      try {
        const content = await withTimeout(fs.readFile(file, 'utf8'), 10000, `dependency_analysis readFile ${file}`);
        const imports = [];
...
export async function entry_point_mapper_handler(args) {
  ...
  try {
    ...
    for (const entry of entryPoints) {
      ...
      try {
        const content = await withTimeout(fs.readFile(entryPath, 'utf8'), 10000, `entry_point_mapper readFile ${entry.file}`);
