...
export async function createFailureIssue(failure) {
  if (_checkVscodeAvailable()) {
    const vscodeMod = _getVscode();
    if (vscodeMod) {
      const enabled = vscodeMod.workspace
        .getConfiguration()
        .get('sweObeyMe.errorReporting.enabled', true);
      if (!enabled) {
...
function _buildPayload(failure) {
  ...
  // Sanitize file paths - remove usernames and local directories
  const sanitizePath = (pathStr) => {
    if (!pathStr || pathStr === '_No file path_') return pathStr;
    // Replace Windows user paths
    let sanitized = pathStr.replace(/C:\\Users\\[^\\]+\\/gi, 'C:\\Users\\[REDACTED]\\');
    // Replace Unix user paths
    sanitized = sanitized.replace(/\/home\/[^/]+\//g, '/home/[REDACTED]/');
    // Replace Users directory
    sanitized = sanitized.replace(/\/Users\/[^/]+\//g, '/Users/[REDACTED]/');
    return sanitized;
  };

  // Sanitize diagnostics - truncate code snippets and remove sensitive data
  const sanitizeDiagnostics = (diag) => {
    if (!diag) return diag;
    let sanitized = diag;
    ...
    sanitized = sanitized.replace(/\/home\/[^/]+\//g, '/home/[REDACTED]/');
    sanitized = sanitized.replace(/\/Users\/[^/]+\//g, '/Users/[REDACTED]/');
    // Truncate very long diagnostics (potential code dumps)
  ...
  // Sanitize backup diffs - completely remove as they contain actual code
  const sanitizeBackupDiff = (diff) => {
    if (!diff || diff === '_No diff available_') return diff;
    return '[REDACTED - Backup diffs contain code and are not sent to GitHub for privacy]';
  };

  // Sanitize router traces - remove paths and truncate
  const sanitizeRouterTrace = (trace) => {
    if (!trace || trace === '_No trace available_') return trace;
    let sanitized = trace;
    sanitized = sanitized.replace(/C:\\Users\\[^\\]+\\/gi, 'C:\\Users\\[REDACTED]\\');
    sanitized = sanitized.replace(/\/home\/[^/]+\//g, '/home/[REDACTED]/');
    sanitized = sanitized.replace(/\/Users\/[^/]+\//g, '/Users/[REDACTED]/');
  ...
  const body = [
    ...
    '## Backup Diff',
    '```diff',
    sanitizeBackupDiff(backupDiff),
    '```',
    '',
    '## Router Trace',
    '```',
    sanitizeRouterTrace(routerTrace),
    '```',
    '',
    '## Environment',
    env,
    ...
    '_Note: File paths, usernames, and code have been redacted for privacy_',
  ].join('\n');
...
async function _logFailureLocally(failure, reason) {
  try {
    const logDir = path.join(os.homedir(), '.sweobeyme-logs');
