...
export const contextHandlers = {
  ...
  get_file_context: async (args) => {
    try {
      ...
      return { isError: true, content: [{ type: "text", text: `Failed to get file context: ${error.message}` }] };
    }
  },
  ...
  analyze_change_impact: async (args) => {
    try {
      ...
      return { isError: true, content: [{ type: "text", text: `Impact analysis failed: ${error.message}` }] };
    }
  },
  ...
  get_symbol_references: async (args) => {
    try {
      ...
      return { isError: true, content: [{ type: "text", text: `Failed to get symbol references: ${error.message}` }] };
    }
  }
};
