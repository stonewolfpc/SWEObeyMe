export function getToolDefinitions() {
  return [
    {
      name: "obey_me_status",
      description: "Checks if the SWEObMe surgical governance system is operational. Use this first to verify the system is active before proceeding with any file operations.",
    },
    {
      name: "obey_surgical_plan",
      description: "CRITICAL: MUST call this BEFORE using write_file to validate your surgical plan complies with architectural rules. This prevents file bloat by checking if your changes will exceed the 700-line limit. If rejected, you MUST use refactor_move_block or extract_to_new_file to reduce file size before proceeding. Example: Call with current_line_count=650 and estimated_addition=100 to check if adding 100 lines to a 650-line file is safe.",
      inputSchema: {
        type: "object",
        properties: {
          target_file: { type: "string", description: "File to be modified - REQUIRED for validation" },
          current_line_count: { type: "number", description: "Current line count - REQUIRED for validation" },
          estimated_addition: { type: "number", description: "Estimated lines to add - defaults to 0 if not specified" },
        },
    ...
    {
      name: "list_directory",
      description: "List files in a directory. Use this to explore project structure before making changes. Example: List the lib directory to see available modules before refactoring.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description: "Get current session memory and history. Use this when you encounter repeated failures or need to understand what actions have been taken. This helps you avoid loops and understand your progress. Example: Check session context after 3 consecutive failures to see if you're stuck in a loop.",
    },
    {
      name: "record_decision",
      description: "Record a decision to session memory for accountability. Use this to document architectural decisions and rationale. Example: Record your decision to extract a large function to a new module for maintainability.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description: "Sanitize a request through SWEObMe filters. This ensures your intent aligns with surgical principles before execution. Use this for complex operations to verify your approach is sound. Example: Sanitize your intent to refactor a large file to ensure it follows surgical principles.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description: "Attempt to repair malformed submissions automatically. Use this when write_file rejects your content with JSON or syntax errors. It fixes trailing commas, markdown wrapping, and removes forbidden patterns. Example: Repair a JSON response that has trailing commas before calling write_file again.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description: "Analyze file health for code smells and complexity. Use this before refactoring to identify issues like deep nesting, silent catch blocks, and complexity. Example: Analyze index.js before refactoring to understand what needs to be improved.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description: "Detect architectural drift from documentation standards. Use this to check if files have adequate documentation (min 10% comment ratio). Example: Check if a recently modified file still follows documentation standards.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    {
      name: "create_backup",
      description: "Create a manual backup of a file. Use this before risky operations. Note: write_file automatically creates backups for existing files, so this is only needed for manual backup operations. Example: Backup a critical file before attempting a complex refactoring.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    {
      name: "restore_backup",
      description: "Restore a file from backup. Use this when a change breaks the code and you need to revert. Example: Restore index.js to its previous state after a failed refactoring attempt. Use backup_index=0 for the most recent backup.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description: "Get status of the active surgical workflow. Use this to track progress and understand which steps are completed. Example: Check workflow status to see if you've completed the extraction step before proceeding to the next step.",
    },
    {
      name: "refactor_move_block",
      description: "Move a code block from one file to another while maintaining surgical compliance. This is the PREFERRED method for reducing file size when obey_surgical_plan rejects your plan. It validates that the target file won't exceed 700 lines. Example: Move a 200-line function from index.js to utils.js to keep index.js under the limit.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description: "Extract a code block to a new file while maintaining surgical compliance. Use this when you need to create a new module from existing code. It validates the new file won't exceed 700 lines and creates a backup of the source. Example: Extract a large class to a new file to reduce source file size.",
      inputSchema: {
        type: "object",
        properties: {
          source_path: { type: "string", description: "Source file path to extract from" },
    ...
    },
    {
      ...
      description: "Get the current architectural directive from SWEObeyMe. Call this when you're unsure about the project's coding standards or when you've encountered multiple failures. This provides the current mandate, integrity score, and compliance status. Example: Call after 3 consecutive failures to understand why your approach is being rejected.",
    },
    {
      name: "request_surgical_recovery",
      description: "Reset session state when you encounter repeated failures. Call this after 3 consecutive errors to clear history and start fresh. This is a recovery mechanism when you're stuck in a loop or the session state is corrupted. Example: Call after 3 failed write attempts to reset and try a different approach.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description: "Query the Oracle for surgical wisdom and motivation. Use this when you need guidance or encouragement during complex tasks. This provides inspirational quotes to maintain surgical discipline. Example: Query the Oracle for motivation when tackling a difficult refactoring.",
    },
    {
      name: "get_config",
      description: "Get current SWEObeyMe configuration values. Use this to view all configurable settings and their current values. Example: Get current configuration to see line count limits and feature toggles.",
    },
    {
      name: "set_config",
      description: "Set SWEObeyMe configuration values. Use this to change settings like line count limits, warning thresholds, debug logging, auto-correction, and feature toggles. All changes are saved to ~/.sweobeyme-config.json. Example: Set maxLines to 800 to increase the file size limit.",
      inputSchema: {
        type: "object",
        properties: {
          settings: {
    ...
    {
      name: "reset_config",
      description: "Reset all SWEObeyMe configuration to default values. Use this when you want to revert all custom settings. Example: Reset configuration after making too many experimental changes.",
    },
    {
      name: "get_config_schema",
      description: "Get the configuration schema with validation rules and descriptions. Use this to understand what configuration options are available and their valid values. Example: Get schema to see what settings can be configured.",
    },
    {
      name: "dry_run_write_file",
      description: "Simulate a write operation without actually writing to the file. CRITICAL for lower-tier models to prevent irreversible mistakes. Validates line count, forbidden patterns, and syntax before any changes. Returns detailed results showing what would happen. MUST use this before write_file when requireDryRun is enabled. Example: Dry run a file write to validate it will succeed.",
      inputSchema: {
        type: "object",
        properties: {
          path: { type: "string", description: "File path to write" },
    ...
    },
    {
      ...
      description: "Comprehensive validation of proposed changes before applying. Checks syntax, imports, anti-patterns, naming conventions, and more. Returns detailed validation report with issues and fixes. CRITICAL for lower-tier models to prevent broken code. Example: Validate a refactoring before applying changes.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description: "Get comprehensive context about a file including imports, exports, functions, classes, and metrics. Provides dependencies and usage information to prevent breaking changes. CRITICAL for understanding ripple effects of changes. Example: Get context for a file before refactoring.",
      inputSchema: {
        type: "object",
        properties: {
          path: { type: "string", description: "File path to analyze" },
    ...
    {
      name: "verify_syntax",
      description: "Validate syntax of JavaScript/TypeScript code. Checks for unmatched braces, parentheses, brackets, and unclosed strings. Returns specific syntax errors with line numbers. CRITICAL for preventing broken code. Example: Verify syntax of code before writing.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description: "Analyze the impact of proposed changes on the codebase. Lists affected files, functions, and classes. Identifies potential breaking changes and dependencies. CRITICAL for understanding ripple effects. Example: Analyze impact before refactoring a function.",
      inputSchema: {
        type: "object",
        properties: {
          path: { type: "string", description: "File path to analyze" },
    ...
    },
    {
      ...
      description: "Enforce strict validation mode with extra conservative checks. Rejects more changes and requires higher quality standards. Use this for lower-tier models that need more guardrails. Example: Enable strict mode for safer operations.",
      inputSchema: {
        type: "object",
        properties: {
