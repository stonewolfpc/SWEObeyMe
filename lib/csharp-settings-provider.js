...
export class CSharpSettingsProvider {
  ...
  async getWebviewContent(webview) {
    ...
    const keepAiInformed = config.get('keepAiInformed', true);
    const severityThreshold = config.get('severityThreshold', 0);
    const confidenceThreshold = config.get('confidenceThreshold', 70);
    const deduplicateAlerts = config.get('deduplicateAlerts', true);
    const alertCooldown = config.get('alertCooldown', 30);
    const logVerbosity = config.get('logVerbosity', 'errors');
    const detectors = config.get('detectors', {});
    ...
    html = html.replace('${keepAiInformed}', keepAiInformed);
    html = html.replace('${severityThreshold}', severityThreshold);
    html = html.replace('${confidenceThreshold}', confidenceThreshold);
    html = html.replace('${deduplicateAlerts}', deduplicateAlerts);
    html = html.replace('${alertCooldown}', alertCooldown);
    html = html.replace('${logVerbosity}', logVerbosity);

    Object.keys(detectors).forEach(key => {
      html = html.replace(`\${detectors.${key}}`, detectors[key]);
    });
