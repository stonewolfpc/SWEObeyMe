...
class BackupManager {
  ...
  async createBackup() {
    ...
    try {
      ...
      await writeFileSafe(manifestPath, JSON.stringify(manifest, null, 2), 10000, 'createBackup write manifest');
  ...
  async backupConfiguration(backupPath) {
    ...
    await writeFileSafe(settingsPath, JSON.stringify(config, null, 2), 10000, 'backupConfiguration write settings');
    ...
    if (await existsSafe(userConfigPath, 1000, 'backupConfiguration exists userConfig')) {
      ...
      let content = await readFileSafe(userConfigPath, 10000, 'backupConfiguration read userConfig');
      ...
      await writeFileSafe(userConfigBackupPath, content, 10000, 'backupConfiguration write userConfig');
    }
  }
  ...
  async backupCheckpoints(backupPath) {
    ...
    await writeFileSafe(path.join(checkpointPath, 'checkpoints.json'), JSON.stringify(checkpointData, null, 2), 10000, 'backupCheckpoints write');
  }
  ...
  async restoreBackup(backupId) {
    ...
    const manifest = JSON.parse(await readFileSafe(manifestPath, 10000, 'restoreBackup read manifest'));
  ...
  async restoreConfiguration(backupPath) {
    ...
    if (await existsSafe(settingsPath, 1000, 'restoreConfiguration exists settings')) {
      const config = JSON.parse(await readFileSafe(settingsPath, 10000, 'restoreConfiguration read settings'));
    ...
    const userConfigBackupPath = path.join(configPath, 'sweobeyme-config.json');
    if (await existsSafe(userConfigBackupPath, 1000, 'restoreConfiguration exists userConfigBackup')) {
      const userConfigPath = path.join(os.homedir(), '.sweobeyme-config.json');
      let content = await readFileSafe(userConfigBackupPath, 10000, 'restoreConfiguration read userConfigBackup');
    ...
    for (const file of files) {
      ...
      if (stats.isDirectory() && file.startsWith('backup-')) {
        ...
        if (await existsSafe(manifestPath, 1000, 'listBackups exists manifest')) {
          try {
            manifest = JSON.parse(await readFileSafe(manifestPath, 10000, 'listBackups read manifest'));
          } catch (error) {
