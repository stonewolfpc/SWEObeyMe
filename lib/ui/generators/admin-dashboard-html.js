...
export function getAdminDashboardHtml(webview) {
  let mcpCfg = {};
  try {
    mcpCfg = JSON.parse(fs.readFileSync(path.join(os.homedir(), '.sweobeyme-config.json'), 'utf8'));
  } catch {}
  const nonce = Math.random().toString(36).slice(2) + Math.random().toString(36).slice(2);
  const cspStyle = `'nonce-${nonce}'`;
  const cspScript = `'nonce-${nonce}'`;
...
<body>
  ...
  <button id="openAdminSettings">Open SWEObeyMe Settings</button>
  ...
  <script ${nonceAttr}>
    ...
    function initUI() {
      ...
      document.getElementById('openAdminSettings').addEventListener('click', function() {
        vscApi.postMessage({command:'openSettings',section:'sweObeyMe'});
      });
    }
