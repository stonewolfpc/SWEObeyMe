...
export async function project_context_handler(params) {
  ...
  switch (operation) {
    case 'detect_type':
      if (!file_path) {
        return {
          isError: true,
          content: [
            { type: 'text', text: 'file_path parameter is required for detect_type operation' },
          ],
        };
      }
    ...
    case 'detect_switch':
      if (!file_path) {
        return {
          isError: true,
          content: [
            { type: 'text', text: 'file_path parameter is required for detect_switch operation' },
          ],
        };
      }
    ...
    case 'switch': {
      if (!project_path || typeof project_path !== 'string' || project_path.trim().length === 0) {
        return {
          ...
          content: [
            {
              type: 'text',
              text: 'project_path parameter is required for switch operation and must be a non-empty string path',
            },
          ],
        };
      }
      const manager = await getProjectAwarenessManager();
      const projectType = await manager.detectProjectType(project_path);
      ...
      const result = await manager.switchProject(projectObj);
        ...
        content: [
          {
            type: 'text',
            text: JSON.stringify(
              { success: true, message: result, projectPath: project_path, projectType },
              null,
              2
            ),
          },
    ...
    default:
      return {
        ...
        content: [
          {
            type: 'text',
            text: `Unknown operation: ${operation}. Valid: detect_type, detect_switch, get_current, get_all, switch`,
          },
        ],
      };
  }
