...
class AutonomousResearchMode {
  ...
  identifyCommonPatterns(query) {
    ...
    if (lowerQuery.includes('database')) {
      patterns.push({
        description:
          'Database pattern: connection pooling, query execution, transaction management',
        relevance: 'high',
      });
    }
    if (lowerQuery.includes('error')) {
      patterns.push({
        description:
          'Error handling pattern: try-catch blocks, error propagation, user-friendly messages',
        relevance: 'high',
      });
    }
