...
export const safeFileOperationsHandlers = {
  ...
  check_file_stats: async (args) => {
    ...
    try {
      ...
      message += `Line Count: ${stats.lineCount}${stats.lineCountTruncated ? ' (truncated — timeout or limit reached)' : ''}\n`;
      message += `Category: ${stats.category}\n`;
      message += `Estimated Read Time: ${stats.estimatedReadTime}ms\n`;
      if (stats.lineCountTruncated) {
        message +=
          '[WARNING] Line count may be partial — file is very large or scan hit safety limit.\n';
      }
  ...
  read_file_chunked: async (args) => {
    ...
    try {
      ...
      if (result.timedOut) {
        message +=
          '\n\n[WARNING] Read timed out — results may be partial. File is extremely large or system is under load.';
      }
