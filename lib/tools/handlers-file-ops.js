...
function generateToolSuggestions(context = {}) {
  ...
  if (context.operation === 'read_file' && context.filePath) {
    suggestions.push('REQUIRED NEXT: Call get_file_context to understand dependencies before modifying.');
    suggestions.push('REQUIRED NEXT: If planning to modify, call obey_surgical_plan with current line count before write_file.');
    suggestions.push('REQUIRED NEXT: Call detect_project_switch if this file is in a different project than your last action.');
  }

  if (context.operation === 'write_file') {
    suggestions.push('REQUIRED NEXT: Call run_related_tests to verify changes did not break anything.');
    suggestions.push('REQUIRED NEXT: Call generate_change_summary to document what was changed.');
    suggestions.push('REQUIRED NEXT: Call record_project_decision if this write reflects a major architectural choice.');
  }

  if (context.operation === 'new_file') {
    suggestions.push('REQUIRED NEXT: Call suggest_file_location to verify correct placement before creating.');
    suggestions.push('REQUIRED NEXT: Call add_pending_task if this file is part of a larger incomplete task.');
  }

  if (context.operation === 'search') {
    suggestions.push('REQUIRED NEXT: Use search_code_files for language-aware ranked results.');
    suggestions.push('REQUIRED NEXT: Call get_file_context on any file you plan to modify after searching.');
  }
  ...
  if (context.operation === 'error' || context.consecutiveFailures > 1) {
    ...
    suggestions.push('REQUIRED NEXT: Call add_project_error to persist this error for future sessions.');
  }

  if (suggestions.length === 0) {
    suggestions.push('REQUIRED: Use obey_me_status at the start of every session.');
    suggestions.push('REQUIRED: Use get_current_project to verify context before significant actions.');
...
export const fileOperationHandlers = {
  ...
  create_backup: async args => {
    try {
      const backupPath = await createBackup(args.path);
      if (!backupPath) {
        return { isError: true, content: [{ type: 'text', text: `Backup failed for ${args.path}. File may not exist.` }] };
      }
      return { content: [{ type: 'text', text: `Backup created: ${backupPath}` }] };
    } catch (error) {
      return { isError: true, content: [{ type: 'text', text: `Backup failed: ${error.message}` }] };
    }
  },
