...
import { detectLanguage, getLangProfile, isJSFamily } from './lang-profile.js';
 ...
 * @param {string} code
 * @param {string} [language] - Detected or explicit language. Defaults to 'javascript'.
 *   Pass the language so JS-only checks don't fire on Go/Python/etc.
 */
export function checkAntiPatterns(code, language) {
  const lang = language || detectLanguage();
  const issues = [];

  // Universal: god functions (language-agnostic heuristic via brace matching)
  if (isJSFamily(lang)) {
  ...
  // Universal: deep nesting (4-space indent → language agnostic)
  const lines = code.split('\n');
  ...
  // Universal: magic numbers (only flag very large digit clusters, not port/year numbers common in all langs)
  const magicNumbers = code.match(/\b\d{6,}\b/g) || [];
  if (magicNumbers.length > 3) {
    issues.push(`Multiple large magic numbers detected: ${magicNumbers.slice(0, 3).join(', ')}...`);
  }

  // Universal: TODO/FIXME (single-line comment style for JS/Go/C family)
  const todos = code.match(/\/\/\s*(TODO|FIXME|HACK|XXX)/gi) || [];
  if (todos.length > 0) {
    issues.push(`${todos.length} TODO/FIXME comments found`);
  }

  // JS/TS only: console.log
  if (isJSFamily(lang)) {
...
/**
 * Validate naming conventions — language-aware.
 * @param {string} code
 * @param {string} [langOrPath] - language id or file path for auto-detection
 */
export function validateNamingConventions(code, langOrPath) {
  const profile = getLangProfile(langOrPath || 'javascript');
  const rules = profile.namingRules;
  const errors = [];
  const warnings = [];

  // Only apply checks where the profile defines a convention
  // and the language actually uses that syntax keyword.

  // Function naming — JS/TS use `function` keyword
  if (rules.functions && isJSFamily(langOrPath)) {
    const functionNames = code.match(/function\s+([a-zA-Z_$][a-zA-Z0-9_$]*)/g) || [];
    functionNames.forEach((fn) => {
      const name = fn.replace('function ', '');
      const startsUpper = name[0] === name[0].toUpperCase() && name[0] !== name[0].toLowerCase();
      if (rules.functions === 'camelCase' && startsUpper) {
        warnings.push(`Function '${name}' should use camelCase`);
      }
    });
  }

  // Class naming — only languages that use the `class` keyword
  const lang =
  ...
  if (
    ...
    classNames.forEach((cls) => {
      ...
      const startsLower = name[0] === name[0].toLowerCase() && name[0] !== name[0].toUpperCase();
      if (rules.classes === 'PascalCase' && startsLower) {
        errors.push(`Class '${name}' should use PascalCase`);
      }
    });
  }

  // Constant naming — JS/TS only (other langs use different declaration syntax)
  if (rules.constants === 'UPPER_SNAKE' && isJSFamily(langOrPath)) {
...
export async function validateCodeComprehensive(code, filePath) {
  const lang = filePath ? detectLanguage(filePath) : 'javascript';
  const results = {
  ...
  if (ENABLE_ANTI_PATTERN_DETECTION()) {
    results.antiPatterns = checkAntiPatterns(code, lang);
    results.overall.warnings.push(...results.antiPatterns.issues);
  }

  // Naming convention validation
  if (ENABLE_NAMING_VALIDATION()) {
    results.naming = validateNamingConventions(code, lang);
