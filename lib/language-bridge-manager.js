...
export async function updateCppDiagnostics(document, diagnosticCollection, __dirname) {
  ...
  // If disabled, only show Windsurf's default diagnostics (clear ours)
  if (!config.get('enabled')) {
    diagnosticCollection.delete(document.uri);
    return;
  }
  ...
  try {
    ...
    const analysisConfig = {
      severityThreshold: config.get('severityThreshold', 0),
    ...
    for (const error of analysis.errors) {
      ...
      for (const range of lineRanges) {
        ...
        const diagnostic = new vscode.Diagnostic(
          range_,
          `[${error.color.toUpperCase()}${toolSource}] ${error.name}: ${typeof error.details === 'object' ? JSON.stringify(error.details) : error.details || 'See details'}`,
          diagnosticSeverity,
        );
    ...
    // Set diagnostics - this ADDS to existing Windsurf diagnostics, doesn't replace
    diagnosticCollection.set(document.uri, diagnostics);
...
export function setupLanguageBridges(context, __dirname) {
  ...
  const cppFileWatcher = vscode.workspace.onDidSaveTextDocument(async (document) => {
    if (['cpp', 'c', 'cuda-cpp'].includes(document.languageId) && document.uri.scheme === 'file') {
...
async function scanExistingFiles(__dirname, csharpDiagnosticCollection, cppDiagnosticCollection) {
  ...
  // Scan C# files
  try {
    const csharpFiles = await vscode.workspace.findFiles('**/*.cs', null, 100);
