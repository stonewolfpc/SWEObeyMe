...
export const errorRules = [
  {
    ...
    check: (content, matches) => {
      ...
      matches.forEach((match) => {
        ...
        if (
          !afterMatch.includes('delete') &&
          !afterMatch.includes('unique_ptr') &&
          !afterMatch.includes('shared_ptr')
        ) {
  ...
  },
  {
    ...
    severity: SeverityLevels.WARNING,
    pattern:
      /^(?!\s*(?:#|\/\/|\/\*|\*|class|struct|namespace|enum|typedef|using))\s*(?:const\s+)?\w+\s+\w+\s*=/gm,
    check: (content, matches) => {
      if (!matches) return null;
  ...
  try {
    ...
    for (const line of lines) {
      const match = line.match(
        /(.+):(\d+):\s+(style|performance|portability|warning|error):\s+(.+)/
      );
      if (match) {
        const severityMap = {
...
export async function analyzeCppFile(filePath, config = {}) {
  const {
    ...
    enabledRules = [
      ...
      'raw_array_new',
    ], // Only enable high-confidence rules by default
      ...
      if (clangErrors) {
        allErrors.push(
          ...clangErrors.map((e) => ({
            ...
            name: 'clang-tidy',
            color:
              e.severity === SeverityLevels.CRITICAL
    ...
    for (const rule of errorRules) {
      ...
      if (result) {
        ...
        lineRanges.forEach((range) => {
          ...
          // Use the specific match text for this range, not the full global result array
          const rangeDetail = range.text
            ? range.text.trim()
...
export async function getIntegrityReport(filePath) {
  ...
  return {
    ...
    integrityScore: Math.max(
      0,
      100 - criticalErrors.length * 10 - warningErrors.length * 5 - infoErrors.length * 2
    ),
...
function generateRecommendations(errors) {
  ...
  if (errors.some((e) => e.id === 'memory_leak_raw_new')) {
    recommendations.push(
      'Consider using std::unique_ptr or std::shared_ptr for automatic memory management'
    );
  }
  ...
  if (errors.some((e) => e.id === 'buffer_overflow_risk')) {
    recommendations.push(
      'Replace unsafe string functions with safer alternatives (strncpy, strncat, snprintf)'
    );
  }
