  ...
  const hiddenTools = [
    {
      ...
      priority: 1,
      description: 'HIDDEN AI-QUERY-ONLY - Query project memory for context. Available query types: snapshot, file_history, decisions, patterns, errors, architecture, backup_history. Not for direct use.',
      inputSchema: {
        type: 'object',
        properties: {
          projectName: { type: 'string', description: 'Project name (optional, uses current project if not provided)' },
          queryType: { type: 'string', description: 'Query type: snapshot, file_history, decisions, patterns, errors, architecture, backup_history' },
          filePath: { type: 'string', description: 'File path (required for file_history and backup_history queries)' },
        },
      },
    },
    {
      ...
      priority: 1,
      description: 'HIDDEN AI-QUERY-ONLY - Query project archives for historical reference. Can list all archives, load specific archive, or search archives. Not for direct use.',
      inputSchema: {
        type: 'object',
        properties: {
          projectName: { type: 'string', description: 'Project name (optional, lists all if not provided)' },
          query: { type: 'string', description: 'Search query (optional, searches archives if provided)' },
        },
      },
    },
    {
      name: 'list_projects',
      priority: 1,
      description: 'HIDDEN AI-QUERY-ONLY - List all projects in the project registry. Not for direct use.',
      inputSchema: {
        type: 'object',
        properties: {},
      },
    },
    {
      ...
      priority: 1,
      description: 'HIDDEN AI-QUERY-ONLY - Query implementation knowledge for experimental attempts, assumptions, working patterns, context annotations, and dependency impacts. Available query types: attempts, assumptions, patterns, annotations, impacts, all. Not for direct use.',
      inputSchema: {
        type: 'object',
        properties: {
          projectName: { type: 'string', description: 'Project name (optional, uses current project if not provided)' },
          queryType: { type: 'string', description: 'Query type: attempts, assumptions, patterns, annotations, impacts, all' },
          filters: { type: 'object', description: 'Filters for query (optional): { outcome, status, annotationType, isNonStandard }' },
          relatedTo: { type: 'string', description: 'Filter to entries related to this file or module (optional)' },
        },
      },
    },
  ];

  return [...criticalTools, ...highPriorityTools, ...mediumPriorityTools, ...hiddenTools].sort((a, b) => (b.priority || 0) - (a.priority || 0));
}
