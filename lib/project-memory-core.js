    ...
    try {
      const data = await Promise.race([
        ...
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error(`Memory file read timeout after ${timeout}ms`)), timeout)
        )
      ]);
  ...
  async loadProjectMap(timeout = 10000) {
    try {
      const data = await Promise.race([
        ...
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error(`Project map read timeout after ${timeout}ms`)), timeout)
        )
      ]);
