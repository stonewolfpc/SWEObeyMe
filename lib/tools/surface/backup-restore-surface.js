...
export async function backupRestoreHandler(params) {
  ...
  if (!operation) {
    return {
      isError: true,
      content: [{ type: 'text', text: 'operation parameter is required (create, restore, list, info, quick_restore)' }],
    };
  }
  ...
  switch (operation) {
    ...
    default:
      return {
        isError: true,
        content: [{ type: 'text', text: `Unknown operation: ${operation}. Valid: create, restore, list, info, quick_restore` }],
      };
  }
