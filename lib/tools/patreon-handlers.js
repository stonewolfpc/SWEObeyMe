...
class PatreonHandlers {
  ...
  async patreonWriteDrafts(args) {
    ...
    try {
      ...
      for (const draft of drafts) {
        ...
        await writeFileSafe(filePath, draft.content, 10000, `patreonWriteDrafts write ${draft.filename}`);
  ...
  async getAPIKey() {
    ...
    if (await existsSafe(configPath, 1000, 'getAPIKey exists configPath')) {
      try {
        const config = JSON.parse(await readFileSafe(configPath, 10000, 'getAPIKey read configPath'));
    ...
    if (await existsSafe(keyFilePath, 1000, 'getAPIKey exists keyFilePath')) {
      try {
        const keyData = JSON.parse(await readFileSafe(keyFilePath, 10000, 'getAPIKey read keyFilePath'));
  ...
  async setAPIKey(apiKey) {
    try {
      ...
      const keyFilePath = join(this.keyDirectory, 'api-key.json');
      await writeFileSafe(keyFilePath, JSON.stringify({
        apiKey,
        createdAt: new Date().toISOString(),
      }, null, 2), 10000, 'setAPIKey write keyFilePath');
