...
export class StreamingFileReader {
  ...
  async initialize() {
    try {
      ...
      const statPromise = fs.stat(this.filePath);
      const statTimeoutPromise = new Promise((_, reject) =>
        setTimeout(() => reject(new Error(`fs.stat timeout for ${this.filePath}`)), 5000)
      );
   ...
   * Uses readline to correctly handle line endings split across chunk boundaries.
   * Added timeout protection to prevent hangs.
   */
  async countLines(timeoutMs = 30000) {
    return new Promise((resolve, reject) => {
      ...
      const timeoutId = setTimeout(() => {
        if (!finished) {
          ...
          stream.destroy();
          rl.close();
          reject(new Error(`countLines timeout after ${timeoutMs}ms for ${this.filePath}`));
        }
      }, timeoutMs);
   ...
   * Added timeout protection to prevent hangs.
   */
  async readChunkFromFile(startLine, lineCount, timeoutMs = 30000) {
    return new Promise((resolve, reject) => {
      const lines = [];
      let currentLine = 0;
      let buffer = '';
      ...
      const timeoutId = setTimeout(() => {
        if (!finished) {
          finished = true;
          stream.destroy();
          reject(new Error(`readChunkFromFile timeout after ${timeoutMs}ms for ${this.filePath}`));
        }
      }, timeoutMs);
  ...
  async write(content, options = {}) {
    try {
      ...
      await new Promise((resolve, reject) => {
        const timeoutId = setTimeout(() => {
          writeStream.destroy();
          reject(new Error(`WriteStream finish timeout for ${this.filePath}`));
        }, 30000);
  ...
  async append(content) {
    try {
      ...
      await new Promise((resolve, reject) => {
        const timeoutId = setTimeout(() => {
          writeStream.destroy();
          reject(new Error(`append WriteStream timeout for ${this.filePath}`));
        }, 30000);
...
export async function writeFileUnlimited(filePath, content, options = {}) {
  ...
  return createSuccessResponse(`Successfully wrote ${result.writtenLines} lines to ${filePath}`);
}
...
export async function searchFileUnlimited(filePath, pattern, options = {}) {
  ...
  return createSuccessResponse(`Found ${results.length} matches for "${pattern}":\n\n${formatted}`);
}
