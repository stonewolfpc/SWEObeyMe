  ...
  return [
    {
      name: 'config_manage',
      priority: 85,
      description: 'Swiss-army-knife for configuration management. Get, set, reset, view schema, and toggle strict mode. Control line count limits, feature toggles, validation strictness, and all other settings. Use this when: changing settings, viewing current configuration, resetting to defaults, or enabling strict mode.',
      inputSchema: {
        type: 'object',
        properties: {
          operation: {
          ...
          settings: {
            type: 'object',
            description: 'Configuration key-value pairs to set (for "set" operation). Valid keys: maxLines, warningThreshold, maxBackupsPerFile, enableAutoCorrection, debugLogs, enableLoopDetection, maxLoopAttempts, minDocumentationRatio, enableWorkflowOrchestration, enableSessionMemory, enableOracle, forbiddenPatterns, reminderInterval',
          },
    ...
    },
    {
      name: 'session_manage',
      priority: 80,
      description: 'Swiss-army-knife for session management. Get context, record decisions, or recover from errors. Use this when: you encounter repeated failures, need to see session history, want to record a decision, or need to recover from errors. Helps break failure loops and maintain audit trails.',
      inputSchema: {
        type: 'object',
        properties: {
          operation: {
    ...
    },
    {
      name: 'code_analyze',
      priority: 75,
      description: 'Swiss-army-knife for code quality analysis. Sanitize requests, repair submissions, analyze file health, detect architectural drift. Use this when: verifying approach is sound, repairing malformed submissions, analyzing file health, or detecting documentation drift.',
      inputSchema: {
        type: 'object',
        properties: {
          operation: {
    ...
    },
    {
      name: 'backup_manage',
      priority: 65,
      description: 'Swiss-army-knife for file safety. Create, restore, or undo backups. Use this before risky operations or when you need to recover from bad edits.',
      inputSchema: {
        type: 'object',
        properties: {
          operation: {
    ...
    },
    {
      ...
      priority: 60,
      description: 'Initiate, track status, or get context for multi-step operations. Use this for tasks requiring 3+ coordinated steps to prevent abandoned half-complete refactors. Helps track complex refactors.',
      inputSchema: {
        type: 'object',
        properties: {
          operation: {
