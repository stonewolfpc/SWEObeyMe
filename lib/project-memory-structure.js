 ...
 */
ProjectMemory.prototype.scanDirectory = async function (dir, excludeDirs = ['.git', 'node_modules', '.vscode'], options = {}) {
  ...
  try {
    ...
    if (e.message && e.message.includes('timeout')) {
      console.warn(`[scanDirectory] Scan timed out after ${timeoutMs}ms, returning ${dirs.length} dirs and ${files.length} files (partial results)`);
    } else {
      throw e;
    }
  }
