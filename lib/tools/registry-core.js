...
export function getCoreToolDefinitions() {
  const criticalTools = [
      ...
      inputSchema: {
        ...
        properties: {
          ...
          override_limit: {
            type: 'number',
            description:
              'Optional: Override the line limit for this check. Use for non-JS projects (Go, C#, Python) where larger files are normal. Alternatively, add .sweobeyme-project.json with {"maxLines": N} in the project root for persistent per-project config.',
          },
        },
    ...
    },
    {
      ...
      priority: 90,
      description:
        'Search the codebase for patterns and symbols. Use this when: finding function definitions, locating usages, searching for specific code patterns. Do NOT use this for: reading file contents, listing directories.',
      inputSchema: {
        type: 'object',
        properties: {
          operation: {
          ...
          language: { type: 'string', description: 'Language to detect (for detect_language)' },
          languages: {
            type: 'array',
            items: { type: 'string' },
            description: 'Languages to filter by',
          },
          filepath: { type: 'string', description: 'File path (for detect_language)' },
    ...
    },
    {
      ...
      priority: 85,
      description:
        'Workflow orchestration and management. Use this when: managing multi-step operations, creating workflows, listing available workflows.',
      inputSchema: {
        type: 'object',
        properties: {
