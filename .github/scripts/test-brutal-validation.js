...
async function testUIComponents() {
  ...
  const webviewChecks = [
    {
      pattern: /registerWebviewViewProvider\(['"]sweObeyMe\.csharpSettings['"]/,
      name: 'Settings webview',
    },
    {
      pattern: /registerWebviewViewProvider\(['"]sweObeyMe\.csharpSettingsView['"]/,
      name: 'C# Bridge webview',
    },
    {
      pattern: /registerWebviewViewProvider\(['"]sweObeyMe\.adminDashboard['"]/,
      name: 'Admin Dashboard webview',
    },
  ];
  ...
  try {
    ...
    for (let i = 0; i < htmlGenerators.length; i++) {
      ...
      if (hasDoctype && hasHtmlTag && hasHeadTag && hasBodyTag && hasClosingTags) {
        ...
        logTest(
          `HTML content validation ${i + 1}`,
          'FAIL',
          `Incomplete HTML structure in ${generator}`
        );
      }
    }
  } catch (e) {
  ...
  try {
    ...
    if (invalidCommands.length > 0) {
      logTest(
        ...
        'WARN',
        `${invalidCommands.length} commands don't follow sweObeyMe. prefix`
      );
    } else {
    ...
    if (commandsMissingTitle.length > 0) {
      logTest(
        ...
        'FAIL',
        `${commandsMissingTitle.length} commands missing title`
      );
    } else if (commandsMissingCategory.length > 0) {
      logTest(
        'Command completeness',
        'WARN',
        `${commandsMissingCategory.length} commands missing category`
      );
    } else {
    ...
    for (const filename of testFiles) {
      ...
      if (platformInvalidChars.test(filename)) {
        logTest(
          ...
          'PASS',
          'Skipped invalid filename (expected behavior)'
        );
        continue;
      }
...
async function testMemoryResources() {
  ...
  for (const pattern of leakPatterns) {
    ...
    if (matches && matches.length > 5) {
      ...
      logTest(
        `Memory leak check: ${pattern.name}`,
        'PASS',
        `Found ${matches?.length || 0} instances`
      );
    }
  }
...
async function testConcurrentOps() {
  ...
  try {
    ...
    if (successCount === 100) {
      ...
      logTest(
        ...
        'WARN',
        `Only ${successCount}/100 operations succeeded`
      );
    }
  } catch (e) {
  ...
  try {
    ...
    if (successCount >= 8) {
      // Allow some failures due to resource limits
      logTest(
        'Concurrent process spawning',
        'PASS',
        `${successCount}/10 processes spawned successfully`
      );
    } else {
  ...
  try {
    ...
    for (const num of boundaryNumbers) {
      ...
      if (Number.isNaN(num)) {
        ...
        const parsed = JSON.parse(encoded);
        decoded =
          parsed.__inf__ !== undefined ? (parsed.__inf__ > 0 ? Infinity : -Infinity) : parsed;
      } else {
        // Normal numbers
