...
import {
  ...
  MIN_DOCUMENTATION_RATIO,
  FORBIDDEN_PATTERNS,
  REQUIRE_DRY_RUN,
  STRICT_MODE
} from "../config.js";
import { dryRunWriteFile, verifySyntax, verifyImports, verifyTypeSafety } from "../verification.js";
...
import { enforceStrictMode, checkForAntiPatterns, checkForDangerousOperations, checkForSecurityIssues } from "../guardrails.js";
import { validateNamingConventions } from "../validation.js";
import { runRelatedTests, checkTestCoverage, hasTests } from "../testing.js";
...
import { confirmDangerousOperation, rateLimitOperation, checkForRepetitivePatterns, validateOperationSafety } from "../safety.js";
...
import { recordFileOperation, wasFileRecentlyWritten, checkForDuplicateWrite } from "../file-operation-audit.js";
...
export const toolHandlers = {
  ...
  read_file: async (args) => {
    ...
    const isCSharp = args.path.endsWith('.cs') || (content.includes('namespace ') && content.includes('using '));
    if (isCSharp) {
      ...
      if (complexity.metrics.maxNestingDepth > 5) warnings.push(`⚠️ DEEP NESTING (${complexity.metrics.maxNestingDepth} levels)`);
      if (complexity.metrics.tryCatchDepth > 3) warnings.push(`⚠️ DEEP TRY-CATCH (${complexity.metrics.tryCatchDepth} levels)`);
      if (!brackets.valid) warnings.push(`⚠️ BRACKET MISMATCH DETECTED`);
      if (complexity.metrics.emptyCatchBlocks.length > 0) warnings.push(`⚠️ ${complexity.metrics.emptyCatchBlocks.length} EMPTY CATCH BLOCKS`);
      if (complexity.metrics.missingUsingStatements.length > 0) warnings.push(`⚠️ ${complexity.metrics.missingUsingStatements.length} MISSING USING STATEMENTS`);
      if (mathSafety.metrics.potentialOverflow.length > 0) warnings.push(`⚠️ ${mathSafety.metrics.potentialOverflow.length} POTENTIAL OVERFLOW RISKS`);
      if (mathSafety.metrics.divisionByZeroRisk.length > 0) warnings.push(`⚠️ ${mathSafety.metrics.divisionByZeroRisk.length} DIVISION BY ZERO RISKS`);
  ...
  write_file: async (args) => {
    ...
    if (operationRecord.status === "DUPLICATE") {
      return {
        isError: true,
        content: [{ type: "text", text: `BLOCKED: Duplicate operation detected. ${operationRecord.note}` }]
      };
    }
    ...
    if (recentDuplicate) {
      console.error(`[SWE-INTEGRITY] Blocked duplicate write to ${args.path} - identical content written ${Date.now() - recentDuplicate.timestamp}ms ago`);
      return {
        isError: true,
        content: [{ type: "text", text: `BLOCKED: Duplicate write operation detected. An identical write was performed on this file ${(Date.now() - recentDuplicate.timestamp) / 1000} seconds ago. This suggests SWE is in a loop. Please review your plan.` }]
      };
    }
    ...
    if (duplicateFiles.length > 0) {
      console.error(`[SWE-INTEGRITY] Detected duplicate file creation attempt: ${args.path} matches ${duplicateFiles.join(', ')}`);
      return {
        isError: true,
        content: [{ type: "text", text: `BLOCKED: File with identical content already exists at: ${duplicateFiles.join(', ')}. Use the existing file instead of creating a duplicate.` }]
      };
    }
    ...
    if (sameNameFiles.length > 0) {
      console.error(`[SWE-INTEGRITY] Detected same-name file creation attempt: ${args.path} matches ${sameNameFiles.join(', ')}`);
      return {
        isError: true,
        content: [{ type: "text", text: `BLOCKED: File with same name already exists in directory: ${sameNameFiles.join(', ')}. This suggests you may be creating a duplicate file. Please verify the file path.` }]
      };
    }
    ...
    if (wasFileRecentlyWritten(args.path, 30000)) {
      ...
      return {
        isError: true,
        content: [{ type: "text", text: `BLOCKED: This file was written less than 30 seconds ago. This suggests SWE may be in a loop. Please wait or verify the operation is necessary.` }]
      };
    }
  ...
  get_workflow_status: async (args) => {
  ...
  get_architectural_directive: async (args) => {
