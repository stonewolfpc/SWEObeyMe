...
export async function readProjectMap(args) {
  try {
    ...
    for (const [boundary, config] of Object.entries(projectMap.conventions.architecturalBoundaries || {})) {
...
export async function writeProjectMap(args) {
  try {
    ...
    return {
      content: [{ type: 'text', text: `Project map updated for ${operation} operation on ${filePath}` }],
    };
  } catch (error) {
    return {
      isError: true,
...
export async function validateAgainstProjectMap(args) {
  try {
    ...
    if (violations.length === 0) {
      return {
        content: [{ type: 'text', text: `✓ File ${filePath} passes all project map validation rules.` }],
      };
    } else {
...
export async function updateDocumentation(args) {
  try {
    ...
    const result = await docUpdater.updateAfterStructuralChange(changeType, filePath, metadata || {});
