...
async function acquireWriteLock(filePath) {
  ...
  if (writeLocks.has(fullPath)) {
    ...
    if (age > BACKUP_LOCK_TIMEOUT) {
      ...
      throw new Error(`File ${filePath} is being written by another operation (lock age: ${age}ms)`);
    }
  }
...
async function performBackup(filePath, fullPath) {
  try {
    ...
    log(`Backup created and verified: ${backupFileName} (hash: ${originalHash.substring(0, 8)}...)`);
    return backupPath;
  } catch (error) {
