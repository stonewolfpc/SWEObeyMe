  ...
  try {
    const content = await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error(`fs.readFile timeout for ${filePath}`)), 6000)
      ),
    ]);
    const errors = [];
