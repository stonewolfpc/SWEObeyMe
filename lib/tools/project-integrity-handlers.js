...
import { getFileRegistry, findDuplicateFiles, findSimilarFiles, findSameNameFiles, searchFiles, getRegistryStatistics } from "../file-registry.js";
import { recordFileOperation, wasFileRecentlyWritten, checkForDuplicateWrite, getAuditStatistics, detectAuditIssues, generateAuditReport } from "../file-operation-audit.js";
import { validateAllReferences, validateImports, validateFileReferences } from "../reference-validation.js";
  ...
  index_project_files: async (args) => {
    try {
      ...
      const result = await registry.indexProject(rootDir, {
        ignorePatterns: args.ignore_patterns || ['node_modules', '.git', 'dist', 'build', 'bin', 'obj', '.next', 'out'],
...
The file registry is now active and will help prevent file duplication.`
      ...
      return { isError: true, content: [{ type: "text", text: `Project indexing failed: ${error.message}` }] };
    }
  },
...
${hasCriticalIssues ? '\n⚠️ CRITICAL: Exact duplicate detected. Consider using existing file or modifying existing content.' : ''}`
      ...
      return { isError: true, content: [{ type: "text", text: `Duplicate check failed: ${error.message}` }] };
    }
  },
...
${hasErrors ? '\n⚠️ CRITICAL: Missing references detected. Ensure all imports/referenced files exist.' : 'All references validated successfully.'}`
      ...
      return { isError: true, content: [{ type: "text", text: `Reference validation failed: ${error.message}` }] };
    }
  },
...
Recommendation: Review the operation history to determine if this operation is necessary.`
      ...
      if (operation && args.content) {
        const duplicate = await checkForDuplicateWrite(file_path, args.content, time_window || 10000);
        if (duplicate) {
          return {
            content: [{
              type: "text",
...
- Blocked: ${stats.blockedCount}`
      ...
      return { isError: true, content: [{ type: "text", text: `Recent operation check failed: ${error.message}` }] };
    }
  },
  ...
  validate_before_write: async (args) => {
    try {
      ...
      if (!file_path || !content) {
        return { isError: true, content: [{ type: "text", text: "file_path and content are required" }] };
      }
...
${issues.filter(i => i.severity !== 'ERROR').length > 0 ? `WARNINGS (${issues.filter(i => i.severity !== 'ERROR').length}):\n${issues.filter(i => i.severity !== 'ERROR').map(i => `  [${i.type}] ${i.message}`).join('\n')}\n` : ''}
...
${hasCriticalIssues ? '❌ WRITE BLOCKED: Critical issues detected. Address these issues before proceeding.' : '✓ VALIDATION PASSED: Safe to write file.'}`
      ...
      return { isError: true, content: [{ type: "text", text: `Pre-write validation failed: ${error.message}` }] };
    }
  },
...
File Types:
${Object.entries(stats.fileTypes).map(([ext, count]) => `  ${ext || 'no-extension'}: ${count}`).join('\n')}
...
${auditIssues.length > 0 ? `Issues Detected:\n${auditIssues.map(i => `  [${i.type}] ${i.message}`).join('\n')}` : 'No issues detected.'}`
      ...
      return { isError: true, content: [{ type: "text", text: `Failed to get registry stats: ${error.message}` }] };
    }
  },
...
${results.length === 0 ? 'No files found matching the pattern.' : ''}`
      ...
      return { isError: true, content: [{ type: "text", text: `File search failed: ${error.message}` }] };
    }
  },
   ...
   */
  generate_audit_report: async (args) => {
    try {
...
${report.recentOperations.map(op => `  [${op.status}] ${op.type} ${op.path} - ${new Date(op.timestamp).toISOString()}`).join('\n')}`
      ...
      return { isError: true, content: [{ type: "text", text: `Audit report generation failed: ${error.message}` }] };
    }
  },
...
Metadata:
  ...
  Hash: ${metadata.hash}` : ''}`
      ...
      return { isError: true, content: [{ type: "text", text: `File existence check failed: ${error.message}` }] };
    }
  }
};
