...
export class ProjectScanner {
  ...
  async detectArchitectureDrift() {
    ...
    const allowedRootFiles = ['README', 'LICENSE', 'package.json', '.gitignore', '.env', 'project_map.json'];
  ...
  async detectMissingDocumentation() {
    ...
    const hasReadme = this.scanResults.files.some(f => 
    ...
    const hasDocsFolder = this.scanResults.folders.some(f => 
      f.toLowerCase() === 'docs'
    );
  ...
  async detectMissingTests() {
    ...
    const hasTestFolder = this.scanResults.folders.some(f => 
    ...
    const testFiles = this.scanResults.files.filter(f => 
  ...
  async detectMissingModuleBoundaries() {
    ...
    const libFileCount = this.scanResults.files.filter(f => f.startsWith('lib') || f.startsWith('src')).length;
