...
function logTest(category, name, status, details = '') {
  ...
  console.log(
    `[${timestamp}] [${category}] ${statusSymbol} ${name}${details ? ` - ${details}` : ''}`
  );
...
async function testServerInitialization() {
  ...
  logTest(
    'Initialization',
    'project-awareness.js exists',
    existsSync(projectAwarenessPath) ? 'PASS' : 'FAIL'
  );
...
async function testToolHandlerExecution() {
  ...
  try {
    const { get_current_project_handler } =
      await import('../lib/tools/project-awareness-handlers.js');
...
async function testCrossPlatformPaths() {
  ...
  try {
    ...
    const pathValid =
      testPath.includes('foo') && testPath.includes('bar') && testPath.includes('baz');
...
async function testEdgeCases() {
  ...
  try {
    const { add_project_error_handler } =
      await import('../lib/tools/project-awareness-handlers.js');
