...
/** Read a file with a 6s timeout to prevent hanging on slow/locked files. */
async function readWithTimeout(filePath, ms = 6000) {
  return Promise.race([
    fs.readFile(filePath, 'utf-8'),
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error(`fs.readFile timeout for ${filePath}`)), ms)
    ),
  ]);
}
...
export const validationHandlers = {
  ...
  validate_naming_conventions: async (args) => {
    try {
      const content = await readFileWithSizeLimit(args.path);
      const lang = args.language || (args.path ? detectLanguage(args.path) : 'javascript');
      const result = validateNamingConventions(content, lang);
      return {
        content: [{ type: 'text', text: JSON.stringify(result, null, 2) }],
      };
    } catch (error) {
  ...
  validate_code: async (args) => {
    try {
        ...
        switch (operation) {
          ...
          case 'verify_syntax':
            return await validationHandlers.verify_syntax({
              code: content || (path ? await readWithTimeout(path) : null),
              language,
            });
          case 'verify_imports':
            return await validationHandlers.verify_imports({
              content: content || (path ? await readWithTimeout(path) : null),
              path,
            });
          case 'check_anti_patterns':
            return await validationHandlers.check_for_anti_patterns({ path, language });
          case 'validate_naming':
            return await validationHandlers.validate_naming_conventions({ path, language });
          case 'comprehensive':
      ...
      const codeContent = content || (path ? await readWithTimeout(path) : null);
      ...
      if (checksToRun.includes('naming')) {
        const namingLang = language || (path ? detectLanguage(path) : 'javascript');
        const result = validateNamingConventions(codeContent, namingLang);
