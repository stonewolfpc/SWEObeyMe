...
// Optional system tools — absence is expected on JS/TS projects, never a GitHub issue
const OPTIONAL_SYSTEM_TOOLS = new Set([
  'ERR-DEPENDENCY-CLANGD',
  'ERR-DEPENDENCY-CLANGTIDY',
  'ERR-DEPENDENCY-CPPCHECK',
  'ERR-DEPENDENCY-DOTNET',
  'ERR-DEPENDENCY-GIT',
  'ERR-DEPENDENCY-NPM',
  'ERR-CLANGD-MISSING',
...
// Optional tool absence is downgraded to stderr warn only — never creates GitHub issues
function reportError(category, code, message, detail) {
  ...
  const isOptional = OPTIONAL_SYSTEM_TOOLS.has(code);
  ...
  process.stderr.write(
    `${categoryTag} ${isOptional ? 'INFO' : category + ' PROBLEM'}: ${message} (${code})\n` +
      `Details: ${detail}\n`
  );

  // Track in health status regardless
