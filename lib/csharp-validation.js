...
import {
  ...
  CSHARP_REQUIRE_ASYNC_AWAIT,
  CSHARP_FORBID_EMPTY_CATCH,
  CSHARP_REQUIRE_USING,
...
export function analyzeCSharpComplexity(code) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('}') && (lines[i - 1]?.trim().includes('catch') || lines[i - 1]?.trim().includes('try'))) {
      currentTryCatchDepth--;
    }
    ...
    const disposablePattern = /\b(Stream|Reader|Writer|Connection|Command|Transaction|HttpClient|DbContext|Disposable)\b/;
    ...
    if (line.includes('if') || line.includes('else if') || line.includes('for') || 
        line.includes('foreach') || line.includes('while') || line.includes('case') ||
 ...
 */
function generateSummary(metrics, issues) {
  const summary = [];

  if (metrics.maxNestingDepth > CSHARP_MAX_NESTING_DEPTH()) {
    summary.push(`CRITICAL: Nesting depth ${metrics.maxNestingDepth} exceeds limit of ${CSHARP_MAX_NESTING_DEPTH()}`);
  }

  if (metrics.maxMethodComplexity > CSHARP_MAX_METHOD_COMPLEXITY()) {
    summary.push(`WARNING: Method complexity ${metrics.maxMethodComplexity} exceeds limit of ${CSHARP_MAX_METHOD_COMPLEXITY()}`);
  }

  if (metrics.tryCatchDepth > CSHARP_MAX_TRY_CATCH_DEPTH()) {
    summary.push(`CRITICAL: Try-catch nesting ${metrics.tryCatchDepth} exceeds limit of ${CSHARP_MAX_TRY_CATCH_DEPTH()}`);
  }
  ...
  if (metrics.missingUsingStatements.length > 0) {
    summary.push(`CRITICAL: ${metrics.missingUsingStatements.length} missing using statement(s) for IDisposable`);
  }

  if (metrics.asyncAwaitIssues.length > 0) {
    summary.push(`WARNING: ${metrics.asyncAwaitIssues.length} async/await anti-pattern(s) detected`);
  }
...
export function validateCSharpBrackets(code) {
  ...
  return {
    ...
    issues,
    summary: issues.length > 0 ? `${issues.length} bracket issue(s) found` : 'All brackets properly matched'
  };
}
...
export function validateCSharpCode(code) {
  ...
  const criticalIssues = allIssues.filter(i => i.type.includes('CRITICAL') || i.type.includes('UNCLOSED') || i.type.includes('UNEXPECTED'));
