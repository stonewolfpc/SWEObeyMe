...
export async function checkPublishReadiness(args) {
  try {
    ...
    let output = validator.formatReport();
...
export async function requestPublishPermission(args) {
  try {
    ...
    if (!validator) {
      return {
        isError: true,
        content: [{ type: 'text', text: 'No publish validator initialized. Use check_publish_readiness first.' }],
      };
    }
    ...
    await import('fs/promises').then(fs => fs.writeFile(statePath, JSON.stringify(state, null, 2), 'utf-8'));
  ...
  try {
    ...
    try {
      ...
      if (!state.requested) {
        return {
          isError: true,
          content: [{ type: 'text', text: 'No publish permission request found. Use request_publish_permission first.' }],
        };
      }
      ...
      return {
        isError: true,
        content: [{ type: 'text', text: 'No publish permission state found. Use request_publish_permission first.' }],
      };
    }
  } catch (error) {
    return {
...
export async function checkPublishPermissionStatus(args) {
  try {
    ...
    return {
      isError: true,
      content: [{ type: 'text', text: `Failed to check publish permission status: ${error.message}` }],
    };
  }
}
