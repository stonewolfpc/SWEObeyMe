...
export async function sweobeymeExecuteHandler(params) {
  ...
  if (!domain) {
    return {
      ...
      content: [
        {
          type: 'text',
          text: 'domain parameter is required (files, backup, search, docs, memory, project, validation, context, safety, config, workflow, refactor, autonomous, agent, csharp, cpp, governance, error, spec, codebase, godot, patreon)',
        },
      ],
    };
  }
  ...
  // Route directly through governance router (with timeout to prevent hangs)
  const timeoutPromise = new Promise((_, reject) =>
    setTimeout(() => reject(new Error('sweobeyme_execute governance router timeout (30s)')), 30000)
  );
