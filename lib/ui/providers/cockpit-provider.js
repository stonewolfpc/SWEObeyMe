...
import { subscribe, getSummary } from '../../health/error-registry.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));

/**
 * Backup state tracker
 * Maintains backup health status and event history
 */
const backupState = {
...
const BACKUP_STALE_THRESHOLD_MS = 10 * 60 * 1000; // 10 minutes

function updateBackupState(event) {
  if (event.type === 'backup') {
    backupState.lastAttempt = {
    ...
    if (event.success) {
      backupState.lastSuccess = {
        timestamp: event.timestamp,
        file: event.file,
        subtype: event.subtype,
      };
    } else {
      backupState.lastFailure = {
        timestamp: event.timestamp,
        file: event.file,
        detail: event.detail,
      };
      backupState.failureCount++;
    }

    // Add to recent events (keep last 50)
    backupState.recentEvents.unshift({ ...event });
    if (backupState.recentEvents.length > 50) {
      backupState.recentEvents.pop();
    }
    ...
    const hasRecentSuccess =
      backupState.lastSuccess &&
      now - backupState.lastSuccess.timestamp < BACKUP_STALE_THRESHOLD_MS;
    const hasSuccessEver = backupState.lastSuccess !== null;
    const hasRecentFailure =
      backupState.lastFailure &&
      now - backupState.lastFailure.timestamp < BACKUP_STALE_THRESHOLD_MS;
    const hasSelfTestError =
      backupState.lastSelfTest && backupState.lastSelfTest.status === 'error';

    if (!event.success && !hasSuccessEver) {
      backupState.status = 'critical';
      backupState.stale = true;
    } else if (hasSelfTestError) {
      backupState.status = 'critical';
      backupState.stale = false;
    } else if (!event.success && hasRecentFailure) {
      backupState.status = 'degraded';
      backupState.stale = true;
    } else if (!hasRecentSuccess && hasSuccessEver) {
      backupState.status = 'degraded';
      backupState.stale = true;
    } else if (event.success && hasRecentSuccess) {
      backupState.status = 'healthy';
      backupState.stale = false;
    } else {
      backupState.status = 'unknown';
      backupState.stale = false;
    }
  } else if (event.type === 'backup-self-test') {
    backupState.lastSelfTest = {
      timestamp: event.timestamp,
      status: event.success ? 'ok' : 'error',
      failures: event.failures || [],
    };

    // Recalculate health status based on self-test results
    if (!event.success) {
      backupState.status = 'critical';
    }
  }
...
function getBackupStatus() {
  return {
    ...backupState,
    recentEvents: backupState.recentEvents.slice(0, 20), // Return last 20 for UI
  };
}
...
export function makeCockpitProvider(context) {
  ...
  // Subscribe to error registry for error status updates
  const unsubscribeErrors = subscribe((errorState) => {
    if (_activeView) {
      _activeView.webview.postMessage({ command: 'errorStatus', ...errorState });
    }
  });
    ...
    async resolveWebviewView(webviewView, _ctx, _token) {
      ...
      registerRouterEventListener((event) => {
        if (_activeView) {
          ...
          // Track backup events
          if (event.type === 'backup') {
            updateBackupState(event);
            _activeView.webview.postMessage({ command: 'backupStatus', backup: getBackupStatus() });
          }
        }
      });
...
async function _handleMessage(context, webview, msg) {
  try {
    switch (msg.command) {
      ...
      case 'getLayerDetail': {
        const detail = LAYER_DETAILS[msg.layerId] || {
          title: msg.layerId,
...
/**
 * Scan backup directory on disk to pre-populate backupState for the current session.
 * Finds the most recent backup/snapshot file and hydrates lastAttempt + lastSuccess.
 */
async function _hydrateBackupStateFromDisk() {
  try {
    const { getDefaultBackupDir } = await import(
      pathToFileURL(path.join(__dirname, '..', '..', 'utils.js')).href
    );
    const backupDir = process.env.SWEOBEYME_BACKUP_DIR || getDefaultBackupDir();

    // Debug: log the backup directory being used
    console.log('[SWEObeyMe] Hydrating backup state from:', backupDir);

    const files = await fs.readdir(backupDir).catch((err) => {
      console.log('[SWEObeyMe] Backup dir read error:', err?.message || 'unknown error');
      return [];
    });

    console.log('[SWEObeyMe] Found', files.length, 'files in backup dir');

    const backupFiles = files
      .filter((f) => f.endsWith('.readonly') && f.includes('.auto-backup-'))
      .sort((a, b) => {
        // Extract timestamps and sort numerically (not alphabetically)
        const tsA = a.match(/\.auto-backup-(\d+)\./)?.[1] || '0';
        const tsB = b.match(/\.auto-backup-(\d+)\./)?.[1] || '0';
        return parseInt(tsB, 10) - parseInt(tsA, 10); // newest first
      });

    console.log('[SWEObeyMe] Found', backupFiles.length, 'backup files');

    if (backupFiles.length > 0) {
      const latest = backupFiles[0];
      const tsMatch = latest.match(/\.auto-backup-(\d+)\./);
      if (tsMatch) {
        const timestamp = parseInt(tsMatch[1], 10);
        backupState.lastAttempt = { timestamp, success: true, file: latest, subtype: 'pre-edit' };
        backupState.lastSuccess = { timestamp, file: latest, subtype: 'pre-edit' };
        backupState.status = 'healthy';
        backupState.stale = Date.now() - timestamp > BACKUP_STALE_THRESHOLD_MS;
        if (backupState.stale) backupState.status = 'degraded';
        // Add to recent events so activity list isn't empty
        backupState.recentEvents.unshift({
          type: 'backup',
          subtype: 'pre-edit',
          success: true,
          file: latest,
        ...
        console.log(
          '[SWEObeyMe] Hydrated backup state:',
          backupState.status,
          new Date(timestamp).toISOString()
        );
      }
    } else {
      console.log('[SWEObeyMe] No backup files found to hydrate');
    }
  } catch (err) {
    // Hydration is best-effort — never throws
    console.error('[SWEObeyMe] Backup hydration failed:', err?.message || err);
  }
}
...
async function _onCockpitReady(webview) {
  ...
  // Push error status summary
  const errorSummary = getSummary();
  webview.postMessage({ command: 'errorStatus', ...errorSummary });

  // Hydrate backup state from disk so UI shows real history on startup
  await _hydrateBackupStateFromDisk();

  // Push backup status
  const backupStatus = getBackupStatus();
  console.log('[SWEObeyMe] Sending backupStatus to client:', JSON.stringify(backupStatus));
  webview.postMessage({ command: 'backupStatus', backup: backupStatus });
}
  ...
  try {
    ...
    webview.postMessage({
      ...
      results: {
        duration: 0,
        tests: [
          { name: 'Validation runner error', status: 'fail', error: err.message, duration: 0 },
        ],
      },
    });
  }
...
async function _testGitHubConnection(webview) {
  ...
  vscode.window.showInformationMessage(
    `SWEObeyMe: Testing connection to ${cfg.owner}/${cfg.repo}…`
  );
  webview.postMessage({
    command: 'routerEvent',
    event: {
      type: 'info',
      message: `GitHub connection test: ${cfg.owner}/${cfg.repo}`,
      timestamp: Date.now(),
    },
  });
}
