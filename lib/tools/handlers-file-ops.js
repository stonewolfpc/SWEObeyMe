...
import { readFileSafe, withTimeout } from '../shared/async-utils.js';
...
function generateToolSuggestions(context = {}) {
  ...
  if (context.operation === 'write_file') {
    suggestions.push(
      'REQUIRED NEXT: Call run_related_tests to verify changes did not break anything.'
    );
    suggestions.push('REQUIRED NEXT: Call generate_change_summary to document what was changed.');
    suggestions.push(
      'REQUIRED NEXT: Call record_project_decision if this write reflects a major architectural choice.'
    );
  }

  if (context.operation === 'new_file') {
    suggestions.push(
      'REQUIRED NEXT: Call suggest_file_location to verify correct placement before creating.'
    );
    suggestions.push(
      'REQUIRED NEXT: Call add_pending_task if this file is part of a larger incomplete task.'
    );
  }

  if (context.operation === 'search') {
    suggestions.push('REQUIRED NEXT: Use search_code_files for language-aware ranked results.');
    suggestions.push(
      'REQUIRED NEXT: Call get_file_context on any file you plan to modify after searching.'
    );
  }

  if (context.operation === 'error' || context.consecutiveFailures > 1) {
    suggestions.push('REQUIRED NEXT: Call get_session_context to review what has been attempted.');
    suggestions.push('REQUIRED NEXT: Call request_surgical_recovery after 3+ consecutive errors.');
    suggestions.push(
      'REQUIRED NEXT: Call add_project_error to persist this error for future sessions.'
    );
  }

  if (suggestions.length === 0) {
    suggestions.push('REQUIRED: Use obey_me_status at the start of every session.');
    suggestions.push(
      'REQUIRED: Use get_current_project to verify context before significant actions.'
    );
    suggestions.push('REQUIRED: Use read_file for all file reads — never access files directly.');
    suggestions.push('REQUIRED: Use write_file for all file writes — never edit files directly.');
  }
...
export const fileOperationHandlers = {
  ...
  read_file: async (args) => {
    // Runtime enforcement: Check if get_governance_constitution was called first
      ...
      if (!sessionTracker.hasCalledTool(sessionId, 'get_governance_constitution')) {
        ...
        const response = {
          ...
          content: [
            {
              type: 'text',
              text: `REQUIRED: You MUST call get_governance_constitution at the START of your session. This establishes the required workflow and governance rules. IMMEDIATELY call get_governance_constitution NOW before proceeding with any file operations.\n\n[SURGICAL INTEGRITY: ${internalAudit.surgicalIntegrityScore}/100 | Consecutive Failures: ${internalAudit.consecutiveFailures}]`,
            },
          ],
        };
        return response;
    ...
    try {
      const content = await readFileSafe(args.path, 30000, 'read_file');
      const stats = await withTimeout(fs.stat(args.path), 5000, 'read_file stat');
      ...
      contextHeader +=
        'Core: lib/tools/ (68 MCP tools), lib/ (validation, enforcement, C# bridge)\n';
      ...
      contextHeader +=
        'Suggested next tools: get_file_context, analyze_change_impact, preflight_change\n';
      contextHeader += '=== END FILE CONTEXT ===\n\n';

      // Shadow Snapshot System: Inject recent edit context
      try {
        const memoryManager = await getProjectMemoryManager('default');
        if (memoryManager) {
          const recentSnapshots = memoryManager.getRecentEditSnapshots(3); // Get last 3 edits
          if (recentSnapshots.length > 0) {
            contextHeader += '=== RECENT EDIT HISTORY ===\n';
            recentSnapshots.forEach((snapshot) => {
              const timeAgo = getTimeAgo(snapshot.timestamp);
              contextHeader += `[${timeAgo}] v${snapshot.versionId?.substring(0, 8) || 'unknown'}: ${snapshot.editSummary}`;
              if (snapshot.context) {
                contextHeader += ` (${snapshot.context})`;
              }
              contextHeader += '\n';
            });
            contextHeader += '=== END EDIT HISTORY ===\n\n';
          }
        }
      } catch (error) {
        console.error('[Shadow Snapshot] Failed to retrieve edit history:', error);
        // Don't fail the read operation if snapshot retrieval fails
      }
      ...
      if (isCSharp) {
        ...
        if (mathSafety.metrics.potentialOverflow.length > 0)
          warnings.push(
            `⚠️ ${mathSafety.metrics.potentialOverflow.length} POTENTIAL OVERFLOW RISKS`
          );
        if (mathSafety.metrics.divisionByZeroRisk.length > 0)
          warnings.push(
            `⚠️ ${mathSafety.metrics.divisionByZeroRisk.length} DIVISION BY ZERO RISKS`
          );
          ...
          if (csharpAnalysis && csharpAnalysis.errorCount > 0 && csharpAnalysis.errors) {
            ...
            if (filteredErrors.length > 0) {
              ...
              filteredErrors.forEach((error) => {
                const severityLabel =
                  error.severity === 2 ? 'CRITICAL' : error.severity === 1 ? 'WARNING' : 'INFO';
      ...
      return injectToolSuggestions(injectRuleCompliance(response, 'read_file', ruleContext), {
        operation: 'read_file',
        filePath: args.path,
      });
    } catch (error) {
  ...
  write_file: async (args) => {
    ...
    if (sessionTracker) {
      // Track this tool call
      sessionTracker.trackToolCall(sessionId, 'write_file');

      if (!sessionTracker.hasCalledTool(sessionId, 'get_governance_constitution')) {
        sessionTracker.recordError(sessionId, 'high');
        internalAudit.consecutiveFailures++;
        ...
        const response = {
          ...
          content: [
            {
              type: 'text',
              text: `REJECTED: You MUST call get_governance_constitution at the START of your session. This establishes the required workflow and governance rules. IMMEDIATELY call get_governance_constitution NOW before proceeding with any file operations. Skipping this will result in rejected changes and lowered integrity scores.\n\n[SURGICAL INTEGRITY: ${internalAudit.surgicalIntegrityScore}/100 | Consecutive Failures: ${internalAudit.consecutiveFailures}]`,
            },
          ],
        };
        return response;
      ...
      // Check if obey_surgical_plan was called before write_file
      if (!sessionTracker.hasCalledTool(sessionId, 'obey_surgical_plan')) {
        sessionTracker.recordError(sessionId, 'high');
        internalAudit.consecutiveFailures++;
        internalAudit.surgicalIntegrityScore -= 10;
        const response = {
          ...
          content: [
            {
              type: 'text',
              text: `REJECTED: You MUST call obey_surgical_plan BEFORE write_file. This is REQUIRED for surgical compliance. IMMEDIATELY call obey_surgical_plan with target_file, current_line_count, and estimated_addition parameters. Your integrity score has been lowered due to this violation.\n\n[SURGICAL INTEGRITY: ${internalAudit.surgicalIntegrityScore}/100 | Consecutive Failures: ${internalAudit.consecutiveFailures}]`,
            },
          ],
        };
        return response;
    ...
    if (operationRecord.status === 'DUPLICATE') {
      ...
      const response = {
        ...
        content: [
          {
            type: 'text',
            text: `BLOCKED: Duplicate operation detected. ${operationRecord.note}\n\n[SURGICAL INTEGRITY: ${internalAudit.surgicalIntegrityScore}/100 | Consecutive Failures: ${internalAudit.consecutiveFailures}]`,
          },
        ],
      };
    ...
    // Shadow Snapshot System: Record edit with task list and version ID
    try {
      const memoryManager = await getProjectMemoryManager('default');
      if (memoryManager) {
        // Generate a version ID for this edit
        const versionId = crypto.randomUUID();
        memoryManager.setVersionId(versionId);

        // Extract task list from session memory if available
        ...
        // Generate a tiny, single-line edit summary
        const lineCount = content.split(/\r\n|\r|\n/).length;
        const editSummary = `Edited ${args.path} (${lineCount} lines)`;

        // Determine context from the edit
        let context = null;
        if (args.explanation) {
          context = args.explanation.substring(0, 100); // Truncate to 100 chars max
        }
      ...
      console.error('[Shadow Snapshot] Failed to record edit snapshot:', error);
      // Don't fail the write operation if snapshot recording fails
    }
    ...
    return injectToolSuggestions(injectRuleCompliance(response, 'write_file', ruleContext), {
      operation: 'write_file',
      filePath: args.path,
    });
  },
  ...
  create_backup: async (args) => {
    try {
      ...
      if (!backupPath) {
        return {
          isError: true,
          content: [{ type: 'text', text: `Backup failed for ${args.path}. File may not exist.` }],
        };
      }
      ...
      return {
        isError: true,
        content: [{ type: 'text', text: `Backup failed: ${error.message}` }],
      };
    }
  },
   ...
   */
  backup_manage: async (params) => {
    const { operation, path, backup_index } = params;
    ...
    switch (operation) {
      case 'create':
        return await fileOperationHandlers.create_backup({ path });
      case 'restore':
        return await fileOperationHandlers.restore_file_from_backup({ path, backup_index });
      case 'undo':
        return await fileOperationHandlers.restore_file_from_backup({ path, backup_index: 0 });
      default:
        return {
          isError: true,
  ...
  /**
   * Dispatcher: code_analyze swiss-army-knife handler
   * Routes to appropriate handler based on operation parameter
   */
  code_analyze: async (params) => {
    ...
    switch (operation) {
      case 'sanitize':
        return {
          content: [{ type: 'text', text: `Sanitized logic intent: ${logic_intent}` }],
        };
      case 'repair':
        return {
          content: [{ type: 'text', text: `Repaired ${type} content` }],
        };
      case 'health':
      ...
      case 'drift':
        return {
          content: [{ type: 'text', text: `Analyzed drift for: ${path}` }],
        };
      default:
        return {
          isError: true,
