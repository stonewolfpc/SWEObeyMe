  ...
  try {
    ...
    for (const view of sweObeyMeViews) {
      const providerPattern = new RegExp(
        `registerWebviewViewProvider\\(['"]${view.id.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}['"]`
      );
...
async function testActivationEvents() {
  ...
  try {
    ...
    if (
      extensionJs.includes('function activate') ||
      extensionJs.includes('async function activate')
    ) {
    ...
    if (
      extensionJs.includes('function deactivate') ||
      extensionJs.includes('async function deactivate')
    ) {
