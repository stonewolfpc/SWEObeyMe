...
class PolicyAsCodeManager {
  ...
  async loadSystemRules() {
    ...
    try {
      if (await existsSafe(directory, 1000, 'loadSystemRules exists directory')) {
        const files = await readdirSafe(directory, {}, 5000, 'loadSystemRules readdir');

        for (const file of files) {
          if (file.endsWith('.md')) {
            const filePath = path.join(directory, file);
            const content = await readFileSafe(filePath, 10000, `loadSystemRules read ${file}`);
  ...
  async loadSystemWorkflows() {
    ...
    try {
      if (await existsSafe(directory, 1000, 'loadSystemWorkflows exists directory')) {
        const files = await readdirSafe(directory, {}, 5000, 'loadSystemWorkflows readdir');

        for (const file of files) {
          if (file.endsWith('.md')) {
            const filePath = path.join(directory, file);
            const content = await readFileSafe(filePath, 10000, `loadSystemWorkflows read ${file}`);
            const name = path.basename(file, '.md');
  ...
  async loadSystemSkills() {
    ...
    try {
      if (await existsSafe(directory, 1000, 'loadSystemSkills exists directory')) {
        const files = await readdirSafe(directory, {}, 5000, 'loadSystemSkills readdir');

        for (const file of files) {
          if (file.endsWith('.md')) {
            const filePath = path.join(directory, file);
            const content = await readFileSafe(filePath, 10000, `loadSystemSkills read ${file}`);
            const name = path.basename(file, '.md');
  ...
  async loadPolicies() {
    ...
    try {
      if (await existsSafe(policyDir, 1000, 'loadPolicies exists policyDir')) {
        const files = await readdirSafe(policyDir, {}, 5000, 'loadPolicies readdir');

        for (const file of files) {
          if (file.endsWith('.json')) {
            const filePath = path.join(policyDir, file);
            const content = await readFileSafe(filePath, 10000, `loadPolicies read ${file}`);
  ...
  async createPolicy(policy) {
    ...
    // Save to disk
    const policyDir = path.join(os.homedir(), '.sweobeyme', 'policies');
    if (!(await existsSafe(policyDir, 1000, 'createPolicy exists policyDir'))) {
      await fs.mkdir(policyDir, { recursive: true });
    }

    const filePath = path.join(policyDir, `${policy.id}.json`);
    await writeFileSafe(filePath, JSON.stringify(policy, null, 2), 10000, 'createPolicy write');
  ...
  async updatePolicy(id, updates) {
    ...
    await writeFileSafe(filePath, JSON.stringify(updated, null, 2), 10000, 'updatePolicy write');
  }
  ...
  async deletePolicy(id) {
    ...
    if (await existsSafe(filePath, 1000, `deletePolicy exists ${id}`)) {
  ...
  evaluateCondition(condition, context) {
    switch (condition.operator) {
      ...
      case 'contains':
        return (
          Array.isArray(context[condition.field]) &&
