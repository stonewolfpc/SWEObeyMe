...
export function requireDocumentation(content) {
  ...
  if (ratio < minRatio) {
    result.valid = false;
    result.errors.push(`Documentation ratio ${(ratio * 100).toFixed(1)}% below minimum of ${(minRatio * 100).toFixed(1)}%`);
  ...
  if (functions.length > documentedFunctions.length) {
    result.warnings.push(`${functions.length - documentedFunctions.length} functions without JSDoc`);
  }
...
export function generateCommitMessage(filePath, changes) {
  ...
  let commitScope = path.basename(filePath, path.extname(filePath));
