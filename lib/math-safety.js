...
import {
  CSHARP_WARN_ON_COMPLEX_MATH,
  CSHARP_MATH_COMPLEXITY_THRESHOLD
} from "./config.js";
...
export function analyzeMathExpression(code) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('int') && (line.includes('*') || line.includes('+'))) {
      ...
      if (hasLargeNumbers) {
        issues.push({
          ...
          severity: 'WARNING',
          message: 'Potential integer overflow detected with large numbers. Consider using long or checked context.'
        });
    ...
    if (line.includes('/') && !line.includes('0') && !line.includes('Zero')) {
      ...
      const hasZeroCheck = previousLines.some(l => 
        l.includes('== 0') || 
        l.includes('!= 0') || 
        l.includes('Zero') ||
        l.includes('throw')
      ...
      if (!hasZeroCheck) {
        issues.push({
          ...
          severity: 'WARNING',
          message: 'Division operation without explicit zero check detected. Add validation to prevent division by zero.'
        });
    ...
    if (line.includes('&&') && line.includes('||')) {
      ...
      if (!hasParens) {
        issues.push({
          ...
          severity: 'WARNING',
          message: 'Mixing && and || operators without parentheses. Use explicit grouping to clarify intent.'
        });
    ...
    if (nestedParens && nestedParens.length > 0) {
      issues.push({
        ...
        severity: 'INFO',
        message: 'Nested parentheses detected. Consider extracting sub-expressions to named variables for clarity.'
      });
    }

    // Detect magic numbers in calculations
    const magicNumbers = line.match(/\b\d+\b/g);
    if (magicNumbers && magicNumbers.length > 2 && !line.includes('const') && !line.includes('readonly')) {
      issues.push({
        line: lineNum,
        type: 'MAGIC_NUMBERS',
        severity: 'INFO',
        message: 'Multiple magic numbers in expression. Consider extracting to named constants for maintainability.'
      });
    }
  }
 ...
 */
function generateMathSummary(metrics, issues) {
  const summary = [];

  if (metrics.complexExpressions.length > 0) {
    const maxComplexity = Math.max(...metrics.complexExpressions.map(e => e.complexity));
    summary.push(`${metrics.complexExpressions.length} complex math expression(s) found (max complexity: ${maxComplexity})`);
  }
  ...
  if (metrics.operatorPrecedenceIssues.length > 0) {
    summary.push(`${metrics.operatorPrecedenceIssues.length} operator precedence issue(s) detected`);
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('==') && (line.includes('double') || line.includes('float'))) {
      issues.push({
        ...
        type: 'FLOAT_EQUALITY',
        severity: 'WARNING',
        message: 'Direct equality comparison of floating point numbers is unreliable. Use Math.Abs(a - b) < epsilon instead.'
      });
    }
    ...
    if (line.includes('Math.Round(') && !line.includes('MidpointRounding')) {
      issues.push({
        ...
        severity: 'INFO',
        message: 'Math.Round without explicit MidpointRounding. Specify rounding mode for predictable behavior.'
      });
    }
    ...
    if (line.includes('double') && line.includes('0.1') || line.includes('0.2') || line.includes('0.3')) {
      issues.push({
        line: lineNum,
        type: 'FLOATING_POINT_IMPRECISION',
        severity: 'WARNING',
        message: 'Floating point arithmetic with values that may have precision issues. Consider using decimal for financial calculations.'
      });
    }
    ...
    if (line.includes('.Select(') || line.includes('.Where(') || line.includes('.Sum(')) {
      ...
      if (nextLines.includes('+') || nextLines.includes('*') || nextLines.includes('/')) {
        issues.push({
          ...
          severity: 'WARNING',
          message: 'Complex calculation within LINQ expression. Consider extracting to separate method for readability and performance.'
        });
      }
    }
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (operators >= 5) {
      suggestions.push({
        ...
        suggestion: 'Extract complex sub-expressions to named variables for better readability and debugging.'
      });
    }
    ...
    if (line.includes('?') && line.includes(':') && (line.includes('>') || line.includes('<'))) {
      suggestions.push({
        ...
        suggestion: 'Consider using Math.Max() or Math.Min() instead of ternary operator for clarity.'
      });
    }

    // Suggest using decimal for financial calculations
    if (line.includes('double') && (line.includes('$') || line.includes('price') || line.includes('amount') || line.includes('cost'))) {
      suggestions.push({
        line: lineNum,
        type: 'USE_DECIMAL',
        suggestion: 'Consider using decimal instead of double for financial calculations to avoid floating point precision issues.'
      });
    }
    ...
    if (line.includes('long') && line.includes('1000000000')) {
      suggestions.push({
        line: lineNum,
        type: 'USE_BIGINT',
        suggestion: 'Consider using BigInteger for very large number calculations to prevent overflow.'
      });
    }
  }
...
export function validateMathSafety(code) {
  ...
  const criticalIssues = allIssues.filter(i => i.severity === 'WARNING' || i.severity === 'CRITICAL');
