...
const internalToolRegistry = {
  ...
  project: {
    ...
    files: 'project_manage',
    init: 'project_manage',
  },
  validation: {
    code: 'validate_code',
    preflight_change: 'preflight_change',
  },
  context: {
  ...
  workflow: {
    manage: 'swe_workflow_manage',
  },
  refactor: {
    manage: 'refactor_manage',
    refactor_manage: 'refactor_manage',
    list: 'refactor_manage',
  ...
  governance: {
    ...
    status: 'governance_manage',
    verify: 'governance_manage',
  },
  error: {
    detect: 'error_detect',
  },
...
export async function governanceRouterHandler(args) {
  ...
  if (!domain) {
    return {
      ...
      next_steps:
        'Provide a domain (files, backup, search, docs, memory, project, validation, context, safety, config, workflow, refactor, autonomous, agent, csharp, cpp, governance, error, spec, codebase, godot, patreon)',
    };
  }
  ...
  if (complianceViolations.length > 0) {
    ...
    const err = {
      code: 'ERR-SURGICAL-COMPLIANCE',
      message: diag,
      detail: `File: ${payload.path}`,
    ...
    createFailureIssue({
      ...
      domain,
      action,
      handlerName: 'checkSurgicalCompliance',
      diagnostics: diag,
    ...
    return {
      ...
      diagnostics: diag,
      next_steps:
        'Fix violations before proceeding. Review SWEObeyMe constitution for surgical rules.',
    };
  }
  ...
  // Emit backup event on every attempt (success or failure)
  if (requiresBackup(domain, action) && payload.path) {
    pushRouterEvent({
      type: 'backup',
      subtype: 'pre-edit',
      ...
      timestamp: Date.now(),
      success: !!backupPath,
      detail: backupPath ? null : 'autoBackupBeforeEdit returned null',
    });
  }
  ...
  if (!domainRegistry) {
    ...
    return {
      ...
      diagnostics: diag,
      next_steps:
        'Valid domains: files, backup, search, docs, memory, project, validation, context, safety, config, workflow, refactor, autonomous, agent, csharp, cpp, governance, error, spec, codebase, godot, patreon',
    };
  }
  ...
  // Call the handler with the payload (with timeout to prevent hangs)
  let result;
  try {
    // Add 30-second timeout to prevent indefinite hangs
    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(() => reject(new Error('Handler execution timeout (30s)')), 30000)
    );
    result = await Promise.race([handler(payload), timeoutPromise]);
    pushRouterEvent({
      type: 'router',
      message: `${domain}.${action} → ${handlerName} ✓`,
      timestamp: Date.now(),
    });
  } catch (error) {
    ...
    if (backupPath && payload.path) {
      try {
        ...
        pushRouterEvent({
          ...
          timestamp: Date.now(),
          success: true,
          detail: `Self-healing: restored ${payload.path}`,
        });
        const err = {
          code: 'ERR-SELF-HEALING',
          message: diag,
          detail: `File: ${payload.path}`,
          source: 'governance-router',
          severity: 'error',
        ...
        return {
          ...
          next_steps:
            'Review the error, fix the issue, and try again. The file has been restored to its pre-edit state.',
        };
      } catch (restoreError) {
        const err = {
          code: 'ERR-BACKUP-RESTORE',
          message: `${diag}. Restore failed: ${restoreError.message}`,
          detail: `File: ${payload.path}, Backup: ${backupPath}`,
          source: 'governance-router',
          severity: 'error',
        };
        registerError(err);
        createFailureIssue({
          ...
          action,
          handlerName,
          diagnostics: `${diag}. Restore failed: ${restoreError.message}`,
          filePath: payload.path,
        }).catch(() => {});
        return {
          status: 'error',
    ...
    const err = {
      code: 'ERR-HANDLER-THROW',
      message: diag,
      detail: `Domain: ${domain}, Action: ${action}, Handler: ${handlerName}`,
      source: 'governance-router',
      severity: 'error',
    };
    registerError(err);
  ...
  if (requiresBackup(domain, action) && payload.path && result && !result.isError) {
    try {
      ...
      log(`Snapshot creation failed: ${error.message}`);
    }
  }
...
export function listGovernanceCapabilities() {
  ...
  return {
    ...
    totalHandlers: Object.values(internalToolRegistry).reduce(
