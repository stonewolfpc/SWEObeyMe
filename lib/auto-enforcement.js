/** Automated Rule Enforcement System — enforces architectural rules proactively */

import fs from 'fs';
import path from 'path';
import { detectLanguage, getDefaultMaxLines, isJSFamily } from './lang-profile.js';
import {
  extractFunctions,
  generateSplitFile,
  groupFunctionsByConcern,
  validateGeneratedCode,
} from './refactoring-engine.js';

/** Walk up from startDir looking for .sweobeyme-project.json maxLines override. */
function loadProjectMaxLines(startDir) {
  let dir = startDir;
  for (let i = 0; i < 8; i++) {
    try {
      const cfg = JSON.parse(fs.readFileSync(path.join(dir, '.sweobeyme-project.json'), 'utf-8'));
      if (cfg && typeof cfg.maxLines === 'number' && cfg.maxLines > 0) return cfg.maxLines;
    } catch {
...
export class AutoEnforcementEngine {
  constructor(options = {}) {
    ...
    // Priority: explicit option > project config > language default (see lang-profile.js)
    const projectMax = options.projectDir ? loadProjectMaxLines(options.projectDir) : null;
    const langMax = options.language ? getDefaultMaxLines(options.language) : null;
    const defaultMax = projectMax || options.maxFileSize || langMax || 700;

    // Default rule thresholds
    this.thresholds = {
      maxFileSize: defaultMax,
  ...
  initializeDefaultRules() {
    ...
    this.registerRule({
      ...
      check: (file, content) => {
        ...
        const langLimit = file ? getDefaultMaxLines(file) : this.thresholds.maxFileSize;
        const effectiveLimit = Math.max(this.thresholds.maxFileSize, langLimit);
        return {
          violated: lineCount > effectiveLimit,
          details: {
            lineCount,
            limit: effectiveLimit,
            language: file ? detectLanguage(file) : 'unknown',
          },
        };
      },
    ...
    // Monolithic file detection — uses language-appropriate function detection
    this.registerRule({
      id: 'monolithic_file',
      ...
      check: (file, content) => {
        const lang = file ? detectLanguage(file) : 'javascript';
        // Count functions using language-appropriate keyword
        const kwMap = {
          go: /\bfunc\s+\w+/g,
          python: /\bdef\s+\w+/g,
          ruby: /\bdef\s+\w+/g,
        ...
        const fnRegex = kwMap[lang] || /function\s+\w+/g;
        const functionCount = (content.match(fnRegex) || []).length;
        const classCount = isJSFamily(lang) ? (content.match(/class\s+\w+/g) || []).length : 0;
        const violated =
          functionCount > this.thresholds.maxFunctionCount ||
          (isJSFamily(lang) && classCount > this.thresholds.maxClassCount);
        return { violated, details: { functionCount, classCount, language: lang } };
      },
  ...
  suggestFileSplit(filePath, content) {
    ...
    // Use the refactoring engine for language-aware extraction
    const functions = extractFunctions(content, filePath);

    if (functions.length === 0) {
      return {
        action: 'split_file',
        message: `File is too large (${lines.length} lines) but no functions could be extracted for splitting.`,
        suggestions: [
          {
            newFile: `${path.basename(filePath, path.extname(filePath))}-split${path.extname(filePath)}`,
            content: '// TODO: Manual split required - could not auto-extract functions',
    ...
    // Group functions by concern for meaningful splits
    const groups = groupFunctionsByConcern(functions);

    // Generate split files
    const suggestions = [];
    for (const [concern, fns] of Object.entries(groups)) {
      if (fns.length === 0) continue;

      const split = generateSplitFile(filePath, content, fns, concern);

      // Validate the generated code
      const validation = validateGeneratedCode(split.content, lang);

      suggestions.push({
        newFile: split.fileName,
        content: split.content,
        lineRange: [fns[0].lineRange[0], fns[fns.length - 1].lineRange[1]],
    ...
    return {
      action: 'split_file',
      message: `File is too large (${lines.length} lines). Split into ${suggestions.length} focused modules by concern.`,
      language: lang,
      suggestions,
    };
  }
  ...
  suggestSeparationOfConcerns(filePath, content) {
    ...
    if (functions.length === 0) {
      return {
        ...
        message:
          'File may handle multiple concerns but no functions could be extracted for analysis.',
        suggestions: [],
      };
    }
    ...
    // Generate full split suggestions with content
    const suggestions = [];
    for (const [concern, fns] of Object.entries(groups)) {
      if (fns.length === 0) continue;
    ...
    return {
      action: 'separate_concerns',
      message: `File handles ${Object.keys(groups).length} different concerns. Separate into focused modules.`,
      language: lang,
