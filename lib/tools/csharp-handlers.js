...
${result.valid ? 'All brackets properly matched!' : `ISSUES (${result.issues.length}):
${result.issues.map(i => `  Line ${i.line}, Column ${i.column}: ${i.message}`).join('\n')}`}`
        }]
      };
    } catch (error) {
      return { isError: true, content: [{ type: "text", text: `Bracket validation failed: ${error.message}` }] };
    }
  },
...
${result.summary.join('\n  ')}`
      ...
      return { isError: true, content: [{ type: "text", text: `Complexity analysis failed: ${error.message}` }] };
    }
  },
...
SUMMARY:
  ${result.summary}`
      ...
      return { isError: true, content: [{ type: "text", text: `Try-catch detection failed: ${error.message}` }] };
    }
  },
...
Legend:
  ...
  ⚠️  - Warning: depth > 5`
      ...
      return { isError: true, content: [{ type: "text", text: `Scope visualization failed: ${error.message}` }] };
    }
  },
...
${result.suggestions.length > 0 ? result.suggestions.map(s => `  Line ${s.line}: ${s.suggestion}`).join('\n') : '  No suggestions'}`
      ...
      return { isError: true, content: [{ type: "text", text: `Math safety validation failed: ${error.message}` }] };
    }
  },
...
${result.summary.join('\n  ')}`
      ...
      return { isError: true, content: [{ type: "text", text: `Math expression analysis failed: ${error.message}` }] };
    }
  },
...
SUMMARY:
  ${result.summary}`
      ...
      return { isError: true, content: [{ type: "text", text: `C# math validation failed: ${error.message}` }] };
    }
  },
...
Note: These are suggestions for improving code quality and maintainability.`
      ...
      return { isError: true, content: [{ type: "text", text: `Math suggestions failed: ${error.message}` }] };
    }
  },
...
${(codeValidation.criticalIssues.length + mathValidation.criticalIssues.length) > 0 ?
  [...codeValidation.criticalIssues, ...mathValidation.criticalIssues].map(i => `  Line ${i.line}: ${i.message}`).join('\n') :
  '  None'}
...
${(codeValidation.warnings.length + mathValidation.infoIssues.length) > 0 ?
  [...codeValidation.warnings, ...mathValidation.infoIssues].map(i => `  Line ${i.line}: ${i.message}`).join('\n') :
  '  None'}
...
${mathValidation.metrics.potentialOverflow.length > 0 ? '  • Add overflow protection for large number calculations' : ''}`
      ...
      return { isError: true, content: [{ type: "text", text: `Health check failed: ${error.message}` }] };
    }
  }
};
