 ...
 * @param {string} configPath - Path to mcp_config.json
 * @param {object} config - Config object to repair
 * @param {Array<object>} issues - Issues to repair
 * @returns {{repaired: boolean, newConfig?: object}}
 */
export async function repairConfig(configPath, config, issues) {
  ...
  if (changed) {
    await fs.writeFile(configPath, JSON.stringify(clone, null, 2), 'utf8');
...
export function getWindsurfConfigPath() {
  ...
  // Try all known editor config paths: Cursor, VS Code, Windsurf variants, generic .codeium
  const possiblePaths = [
  ...
  // Return the default if none exist (windsurf-next preferred)
