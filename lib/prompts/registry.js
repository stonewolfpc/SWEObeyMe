...
export const PROMPT_METADATA_SCHEMA = {
  ...
  category: {
    type: 'string',
    enum: ['governance', 'architecture', 'project-awareness', 'preflight', 'rewrite', 'workflow', 'debugging', 'skills'],
  },
  persona: {
    type: 'string',
...
class PromptRegistry {
  ...
  getStatistics() {
    return {
      ...
      skillIntegratedPrompts: Array.from(this.prompts.values()).filter(p => p.skills && p.skills.length > 0).length,
    };
  }
  ...
  interpolateTemplate(template, context) {
    ...
    result = result.replace(/{{#if\s+\(not\s+(\w+)\)}}([\s\S]*?){{\/if}}/g, (match, varName, content) => {
      return !context[varName] ? content : '';
    });
  ...
  getPromptsBySkill(skillName) {
    return Array.from(this.prompts.values()).filter(prompt =>
      prompt.skills && prompt.skills.includes(skillName)
    );
  }
}
