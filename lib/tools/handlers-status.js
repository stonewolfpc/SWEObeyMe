...
import { DEBUG_LOGS, MAX_LINES } from '../config.js';
...
export const statusHandlers = {
  ...
  obey_surgical_plan: async (args) => {
    ...
    // Resolve effective line limit (priority: override_limit arg > .sweobeyme-project.json > global config > 700)
    let effectiveLimit = MAX_LINES ? MAX_LINES() : 700;
    let limitSource = 'global config';

    // Check for per-project config file in the file's directory or any ancestor
    if (args.override_limit && typeof args.override_limit === 'number' && args.override_limit > 0) {
      effectiveLimit = args.override_limit;
      limitSource = 'override_limit param';
    } else {
      try {
        const targetDir = path.dirname(path.resolve(args.target_file));
        let searchDir = targetDir;
        for (let depth = 0; depth < 8; depth++) {
          const projectConfigPath = path.join(searchDir, '.sweobeyme-project.json');
          try {
            const raw = fs.readFileSync(projectConfigPath, 'utf-8');
            const projectConfig = JSON.parse(raw);
            if (projectConfig.maxLines && typeof projectConfig.maxLines === 'number') {
              effectiveLimit = projectConfig.maxLines;
              limitSource = `.sweobeyme-project.json (${projectConfigPath})`;
              break;
            }
          } catch {
            // No config at this level, check parent
          }
        ...
        // Path resolution failed — use global limit
      }
    }
    ...
    if (total > effectiveLimit) {
      ...
      log(`CRITICAL: ${args.target_file} will exceed ${effectiveLimit} lines. SPLIT REQUIRED.`);
      return {
        isError: true,
        content: [
          {
            type: 'text',
            text: `REJECTED: File bloat detected. Current: ${args.current_line_count} lines. After addition: ${total} lines (exceeds ${effectiveLimit} limit from ${limitSource}). ACTION REQUIRED: Use 'refactor_move_block' or 'extract_to_new_file' to reduce file size before proceeding. TIP: For non-JS projects, add a .sweobeyme-project.json with {"maxLines": N} in the project root, or pass override_limit=N to this tool.\n\n[SURGICAL INTEGRITY: ${internalAudit.surgicalIntegrityScore}/100 | Consecutive Failures: ${internalAudit.consecutiveFailures}]`,
          },
        ],
      };
    }
    ...
    return {
      content: [
        {
          type: 'text',
          text: `PLAN APPROVED: ${args.current_line_count} + ${args.estimated_addition || 0} = ${total} lines (within ${effectiveLimit} limit from ${limitSource}). Proceed with surgical precision.\n\n[SURGICAL INTEGRITY: ${internalAudit.surgicalIntegrityScore}/100]`,
        },
      ],
    };
  },
