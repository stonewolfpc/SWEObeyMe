...
class TransportSentinel extends EventEmitter {
  constructor(options = {}) {
    ...
    // Inflight request tracking — recovery must never fire during active requests
  ...
  /**
   * Mark the start of an inflight MCP request.
   * Recovery is suppressed while any request is active.
   */
  markRequestStart() {
  ...
  /**
   * Mark the end of an inflight MCP request.
   */
  markRequestEnd() {
    this._inflightCount = Math.max(0, this._inflightCount - 1);
    this.lastResponseTime = Date.now();
  ...
  /**
   * Trigger recovery from failure — suppressed when requests are inflight.
   */
  triggerRecovery() {
    if (this._inflightCount > 0) {
      process.stderr.write(
        `[SWEObeyMe] TransportSentinel: Recovery suppressed — ${this._inflightCount} request(s) inflight.\n`
      );
      return;
    }
