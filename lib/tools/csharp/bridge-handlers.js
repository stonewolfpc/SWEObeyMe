...
export const csharpBridgeHandlers = {
  get_csharp_errors: async args => {
    try {
      ...
      // Analyze each file
      const allErrors = [];
      for (const filePath of csFiles.slice(0, 50)) { // Limit to prevent timeouts
      ...
      return {
        content: [{
          ...
          text: `Found ${allErrors.length} C# errors in workspace:\n\n${
            allErrors.map(e => `[${e.severity === 2 ? 'Error' : e.severity === 1 ? 'Warning' : 'Info'}] ${e.fileName}:${e.line}: ${e.message}`).join('\n')
          }`
        }],
        errors: allErrors,
        ...
        content: [{
          ...
          text: filtered.length > 0 
            ? `C# errors in ${path.basename(args.path)}:\n\n${
                filtered.map(e => `[${e.severity === 2 ? 'Error' : e.severity === 1 ? 'Warning' : 'Info'}] Line ${e.line}: ${e.message}`).join('\n')
              }`
