...
export const cppBridgeHandlers = {
      ...
      return {
        content: [{
          ...
          text: `Found ${allErrors.length} C++ errors in workspace:\n\n${
            allErrors.map(e => `[${e.severity === 2 ? 'Error' : e.severity === 1 ? 'Warning' : 'Info'}] ${e.fileName}:${e.lineRanges?.[0]?.startLine || '?'}: ${e.name}`).join('\n')
          }`
        }],
        errors: allErrors,
        ...
        content: [{
          ...
          text: filtered.length > 0 
            ? `C++ errors in ${path.basename(args.path)}:\n\n${
                filtered.map(e => `[${e.severity === 2 ? 'Error' : e.severity === 1 ? 'Warning' : 'Info'}] Line ${e.lineRanges?.[0]?.startLine || '?'}: ${e.name}: ${typeof e.details === 'object' ? JSON.stringify(e.details) : e.details}`).join('\n')
              }`
  ...
  get_cpp_integrity_report: async args => {
    ...
    try {
      ...
      return {
        content: [{
          type: 'text',
          text: `Surgical Integrity Report for ${path.basename(args.path)}:\n\n` +
  ...
  toggle_cpp_error_type: async args => {
    ...
    try {
      ...
      if (args.enabled) {
        global.cppConfig.disabledRules = global.cppConfig.disabledRules.filter(id => id !== args.error_id);
      } else {
  ...
  set_cpp_ai_informed: async args => {
    ...
    try {
      ...
      return {
        content: [{
          type: 'text',
          text: `AI error notification ${args.enabled ? 'enabled' : 'disabled'}\n\n` +
  ...
  update_cpp_config: async args => {
    try {
      ...
      return {
        content: [{
          type: 'text',
          text: 'C++ Bridge configuration updated successfully\n\n' +
    ...
    try {
      ...
      return {
        content: [{
          type: 'text',
          text: result.success
            ? `Restored ${path.basename(args.path)} from backup\n\nTimestamp: ${result.timestamp}`
      ...
      return {
        content: [{ type: 'text', text: `Error restoring file: ${error.message}` }],
        isError: true,
        error: error.message
      };
    }
