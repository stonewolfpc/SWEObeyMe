...
export function getClientScript() {
  return `
    ...
    function updateBackupIndicator() {
      ...
      // Update status text and dot if elements exist
      if (statusEl) {
    ...
    function updateBackupActivity() {
      ...
      activityEl.innerHTML = backupStatus.recentEvents.map(ev => {
        ...
        const file = ev.file ? _esc(ev.file.replace(/\\/g, '/').split('/').pop() || ev.file) : 'unknown';
    ...
    try {
      ...
      setupSettings();
      setupErrorModal();
      // Refresh relative backup timestamps every 60s
