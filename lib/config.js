...
export async function loadConfig() {
  try {
    if (await fs.access(CONFIG_FILE_PATH).then(() => true).catch(() => false)) {
      ...
      console.log(`[CONFIG] Loaded configuration from ${CONFIG_FILE_PATH}`);
    } else {
...
export async function saveConfig() {
  try {
    ...
    console.log(`[CONFIG] Saved configuration to ${CONFIG_FILE_PATH}`);
    return true;
  } catch (error) {
...
export function setConfig(key, value) {
  ...
  if (typeof value !== typeof defaultValue) {
    throw new Error(`Invalid type for ${key}: expected ${typeof defaultValue}, got ${typeof value}`);
  }
