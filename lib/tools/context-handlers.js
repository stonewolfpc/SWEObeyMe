        ...
        const gitLog = await new Promise((resolve, reject) => {
          const childProcess = exec(
            ...
            (error, stdout, stderr) => {
              if (error) {
                console.error(
                  `[SWEObeyMe-Git] Git command failed for ${filePath}: ${error.message}, stderr: ${stderr}`
                );
                reject(error);
              } else {
                resolve(stdout);
          ...
          // If childProcess.kill is not available, then this is not Node.js
          if (childProcess && typeof childProcess.kill === 'function') {
            const timeoutId = setTimeout(() => {
              childProcess.kill(); // Terminate the child process
              reject(new Error('Git operation timeout after 10 seconds'));
            }, 10000);

            childProcess.on('exit', () => clearTimeout(timeoutId));
