...
export class ProjectAwarenessManager {
  constructor() {
    ...
    // ensureRegistryExists() is now async and called in initialize()
    // Do NOT call initialize() here - it causes double-initialization deadlock
  }

  /**
   * Ensure the project registry directory exists (async version called in initialize)
   */
  async ensureRegistryExists() {
    try {
      ...
      console.error('[Project Awareness] Failed to ensure registry exists:', error);
    }
  }
  ...
  /**
   * Load projects from registry file into the projects Map
   */
  async loadProjects() {
    try {
      if (await existsSafe(PROJECTS_FILE, 1000, 'loadProjects exists')) {
        const data = await readFileSafe(PROJECTS_FILE, 10000, 'loadProjects read');
    ...
    while (currentPath !== path.dirname(currentPath) && depth < maxDepth) {
      ...
      for (const indicator of indicators) {
        try {
          if (
            await fs
              .access(path.join(currentPath, indicator))
              .then(() => true)
              .catch(() => false)
          ) {
            return currentPath;
