      ...
      const uiTests = [
        ...
        },
        {
          ...
          test: () => {
            ...
            const windowsPaths = (htmlContent.match(/[A-Z]:[\\/][^"'\\s]*\.(js|json|md|txt|png|svg)/g) || []).length;
            const unixPaths = (htmlContent.match(/\/[^"'\\s]*\.(js|json|md|txt|png|svg)/g) || []).length;
...
async function testWebviewProviders() {
  try {
    ...
    logTest('Webview providers found', providers.length > 0 ? 'PASS' : 'FAIL', `${providers.length} providers`);
    ...
    const webviewTests = [
      {
        ...
        test: extensionJs.includes('enableScripts: true') || extensionJs.includes('webview.options'),
        critical: true
      },
      {
...
async function testCommandUIIntegration() {
  try {
    ...
    const uiCommands = commands.filter(cmd => 
    ...
    logTest('UI-related commands', uiCommands.length > 0 ? 'PASS' : 'FAIL', `${uiCommands.length} UI commands`);
  ...
  try {
    ...
    const errorTests = [
      ...
      },
      {
        ...
        test: extensionJs.includes('showErrorMessage') || extensionJs.includes('showWarningMessage'),
        critical: true
      },
      {
        name: 'Fallback error UI',
        test: extensionJs.includes('fallback') || extensionJs.includes('error') && extensionJs.includes('html'),
        critical: false
      },
      {
