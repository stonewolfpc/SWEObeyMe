...
export async function recordChange(args) {
  try {
    ...
    return {
      content: [{ type: 'text', text: `Change recorded: [${change.type.toUpperCase()}] ${change.description}` }]
    };
  } catch (error) {
    return {
      isError: true,
...
export async function clearChanges(args) {
  try {
    ...
    return {
      content: [{ type: 'text', text: '✅ Changes cleared after successful push. Version state reset.' }]
    };
  } catch (error) {
    return {
      isError: true,
